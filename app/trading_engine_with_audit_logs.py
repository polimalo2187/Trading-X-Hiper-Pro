    ...
    try:
        ...
        if ok:
            log(
                ...
                f"sl_price={float(sl_trigger):.8f} qty={float(qty_coin):.8f} trigger={sl_resp.get('triggerPx')} status={sl_resp.get('reason')} current={current_px if current_px > 0 else 'n/a'}",
                "WARN",
            )
            return True
...
def _coalesce_management_params(
    *,
    ...
    # entry_strength / best_score se mantienen por compatibilidad de firma,
    # pero el engine no calcula más la gestión del trade.
    _ = entry_strength
    _ = best_score
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    ...
    adopt_mgmt = _coalesce_management_params(active_trade=_get_active_trade(user_id))

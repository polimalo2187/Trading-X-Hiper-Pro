...
# ============================================================
# POSICIÓN REAL (signed size) — fuente de verdad del exchange
# ============================================================

def _get_exchange_position_szi(user_id: int, coin: str) -> float:
    """Retorna tamaño de posición REAL (signed) desde el exchange:
    >0 = long, <0 = short, 0 = sin posición.
    """
    try:
        coin_n = _norm_coin(coin)
        wallet = get_user_wallet(user_id)
        if not wallet:
            return 0.0
        payload = {"type": "clearinghouseState", "user": wallet}
        state = make_request(user_id, "info", payload) or {}
        aps = state.get("assetPositions") or []
        for ap in aps:
            pos = (ap or {}).get("position") or {}
            if str(pos.get("coin", "")).upper() == coin_n:
                return float(pos.get("szi", 0.0) or 0.0)
        return 0.0
    except Exception:
        return 0.0

def _is_reduce_only_increase_err(resp: Any) -> bool:
    try:
        msg = str(_resp_reason(resp) or "").lower()
        return ("reduce only" in msg and "increase position" in msg) or ("reduce-only" in msg and "increase position" in msg)
    except Exception:
        return False
...
# -----------------------------
# Persistencia institucional (TP dinámico) — MongoDB
# -----------------------------
_MONGO_CLIENT = None
_MONGO_DB = None
_POS_STATE_COL = "position_state"

def _get_mongo_db():
    """Devuelve el db de Mongo (pymongo) usando env MONGO_URL / DB_NAME.
    No falla el bot si Mongo no está disponible: retorna None.
    """
    global _MONGO_CLIENT, _MONGO_DB
    if _MONGO_DB is not None:
        return _MONGO_DB
    mongo_url = os.getenv("MONGO_URL") or os.getenv("MONGODB_URI") or os.getenv("MONGO_URI")
    db_name = os.getenv("DB_NAME") or os.getenv("MONGO_DB") or "trading_bot"
    if not mongo_url:
        return None
    try:
...
def _load_tp_peak_pct(user_id: int, symbol: str) -> float | None:
    db = _get_mongo_db()
    if db is None:
        return None
    doc = db[_POS_STATE_COL].find_one({"user_id": int(user_id), "symbol": str(symbol)})
    if not doc:
        return None
    val = doc.get("tp_peak_pct")
    try:
        return float(val)
    except Exception:
        return None

def _save_tp_peak_pct(user_id: int, symbol: str, direction: str, tp_peak_pct: float) -> None:
    db = _get_mongo_db()
    if db is None:
        return
    try:
        db[_POS_STATE_COL].update_one(
            {"user_id": int(user_id), "symbol": str(symbol)},
            {
                "$set": {
                    "user_id": int(user_id),
                    "symbol": str(symbol),
                    "direction": str(direction),
                    "tp_peak_pct": float(tp_peak_pct),
...
def _clear_tp_peak(user_id: int, symbol: str) -> None:
    db = _get_mongo_db()
    if db is None:
        return
    try:
        db[_POS_STATE_COL].delete_one({"user_id": int(user_id), "symbol": str(symbol)})
    except Exception:
        return


# -----------------------------
# PnL institucional — usar fills reales del exchange
# -----------------------------
def _extract_fills_from_order_resp(resp: dict) -> list[dict]:
    """Intenta extraer fills desde la respuesta del exchange (Hyperliquid).
    ...
    try:
        ...
        fills = []
        for st in statuses:
            if isinstance(st, dict) and "filled" in st and isinstance(st["filled"], dict):
                f = st["filled"]
                # Algunos endpoints devuelven una lista 'fills'
                if isinstance(f.get("fills"), list):
                    fills.extend([x for x in f["fills"] if isinstance(x, dict)])
                else:
                    fills.append(f)
        return fills
    except Exception:
...
def _fills_avg_px_and_sz(fills: list[dict]) -> tuple[float | None, float | None]:
    """Promedio ponderado de precio y tamaño a partir de fills."""
    if not fills:
        return (None, None)
    total_sz = 0.0
    total_notional = 0.0
    for f in fills:
        px = f.get("px") or f.get("price") or f.get("avgPx")
        sz = f.get("sz") or f.get("size") or f.get("totalSz") or f.get("filledSz")
        try:
            px = float(px)
            sz = float(sz)
        except Exception:
...
def _compute_realized_pnl_usdc(
    entry_px: float,
    direction: str,
    close_resp: dict,
    fallback_sz: float,
) -> tuple[float | None, float | None, float | None]:
    """Retorna (pnl_usdc, exit_px, exec_sz) usando fills reales.
    fallback_sz: tamaño (coins) del position, por si el resp no trae sz.
    """
    fills = _extract_fills_from_order_resp(close_resp)
    exit_px, exec_sz = _fills_avg_px_and_sz(fills)
    if exit_px is None:
        # fallback a extractor viejo (si existía) — precio
        try:
            exit_px = float(_extract_fill_price(close_resp))
        except Exception:
            exit_px = None
    if exec_sz is None:
        exec_sz = float(fallback_sz) if fallback_sz is not None else None

    if exit_px is None or exec_sz is None:
        return (None, exit_px, exec_sz)

    if direction == "long":
        pnl = (exit_px - float(entry_px)) * float(exec_sz)
    else:
        pnl = (float(entry_px) - exit_px) * float(exec_sz)
    return (pnl, exit_px, exec_sz)
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    """Gestiona una posición ya abierta en el exchange (SL/TRAIL) hasta cerrarla."""
    ...
    try:
        size_signed = float(_get_exchange_position_szi(user_id, symbol_for_exec) or 0.0)
    except Exception:
        size_signed = 0.0
    ...
    log(
        f"🧠 MANAGER: tomando posición abierta {symbol} dir={direction} "
    ...
    log(f"Cerrando posición (MANAGER) {symbol} reason={exit_reason}")

    # cerrar por size REAL + reduceOnly
    try:
        size_signed_now = float(_get_exchange_position_szi(user_id, symbol_for_exec) or 0.0)
    except Exception:
        size_signed_now = 0.0
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        size_real_signed = float(_get_exchange_position_szi(user_id, symbol_for_exec) or 0.0)
        ...
        # ✅ Cierre BANK-GRADE: cerrar por size REAL del exchange + reduceOnly
        try:
            size_signed = float(_get_exchange_position_szi(user_id, symbol_for_exec) or 0.0)
        except Exception:
            size_signed = 0.0
        ...
        if close_qty <= 0:
            close_qty = float(qty_coin)  # fallback (no debería pasar)

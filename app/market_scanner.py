...
def get_all_24h_stats():
    ...
    for asset in asset_ctxs:
        ...
        if not symbol:
            safe_log("⚠ Asset sin coin key, se ignora")
            continue
        ...
        # Hyperliquid usa PERP explícito
...
def analyze_symbol(symbol: str, stats: dict):
    ...
    try:
        ...
        # FIX CRÍTICO:
        # Hyperliquid suele devolver prevDayPx = None
        raw_prev = info.get("prevDayPx")
        prev_price = float(raw_prev) if raw_prev not in (None, 0, "0") else price

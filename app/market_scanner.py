...
def get_all_24h_stats():
    """
    Devuelve activos PERP reales de Hyperliquid.
    Usa símbolos simples: BTC, ETH, SOL...
    """
    ...
    for item in asset_ctxs:
        ...
        stats_map[symbol.upper()] = item

    return stats_map


# ============================================================
# 2. ANALIZAR ACTIVO (SCORE GUERRA REAL)
# ============================================================
...
def analyze_symbol(symbol: str, stats: dict) -> dict | None:
    ...
    # ========================================================
    # SCORE GUERRA (ADAPTADO A SCALPING REAL)
    # ========================================================

    vol_score = min(volume / 250_000, 1.0)
    oi_score = min(oi / 1_000_000, 1.0)
    trend_score = max(min((change / 3) + 0.5, 1.0), 0.0)
    spread_score = 1 - min(spread / (price * 0.004), 1.0)
...
def get_best_symbol() -> dict | None:
    ...
    safe_log(
        f"🔥 ACTIVO SELECCIONADO | {best['symbol']} | "
        f"Score={best['score']} | "

...
# --- Entry timing (5m) – Trend + Pullback confirmado (bloqueador)
EMA_ENTRY_5M = 20
MAX_EXT_PCT_5M = 0.008  # 0.8% (ajustable)
PULLBACK_LOOKBACK_5M = 18
ADX_PERIOD = 14
ADX_MIN_TREND_1H = 25
...
def _pullback_confirmed_5m(direction: str, h5: List[float], l5: List[float], cl5: List[float]) -> Tuple[bool, str]:
    """
    Bloqueador de timing (5m):
      - Evita entrar extendido lejos de EMA20.
      - Exige que haya habido un 'touch' a EMA20 recientemente y confirmación (close actual al lado correcto).
    Devuelve (ok, reason_if_not_ok).
    """
    try:
        ema20_series = _ema(cl5, EMA_ENTRY_5M)
        ema20_last = _last(ema20_series)
        if ema20_last is None or float(ema20_last) <= 0:
            return False, "NO_EMA20_5M"

        close_now = float(cl5[-1])

        # 1) Anti-entrada tardía (extendido)
        if direction == "LONG":
            ext = (close_now - float(ema20_last)) / float(ema20_last)
        else:
            ext = (float(ema20_last) - close_now) / float(ema20_last)

        if ext > float(MAX_EXT_PCT_5M):
            return False, "EXTENDED_5M"

        # 2) Pullback: hubo touch a EMA20 recientemente
        lb = max(5, min(int(PULLBACK_LOOKBACK_5M), len(cl5)))
        start = max(0, len(cl5) - lb)

        touched = False
        for i in range(start, len(cl5)):
            e = ema20_series[i]
            if e is None or float(e) <= 0:
                continue
            if direction == "LONG":
                if float(l5[i]) <= float(e):
                    touched = True
                    break
            else:
                if float(h5[i]) >= float(e):
                    touched = True
                    break

        if not touched:
            return False, "NO_PULLBACK_5M"

        # 3) Confirmación mínima (cierre actual del lado correcto de EMA20)
        if direction == "LONG":
            if close_now <= float(ema20_last):
                return False, "NO_CONFIRM_5M"
        else:
            if close_now >= float(ema20_last):
                return False, "NO_CONFIRM_5M"
        ...
        return False, "PULLBACK_CHECK_FAIL"
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # ✅ Bloqueador de timing (Trend + Pullback confirmado en 5m)
        ok_pb, pb_reason = _pullback_confirmed_5m("LONG" if direction == "LONG" else "SHORT", h5, l5, cl5)
        if not ok_pb:
            return {"signal": False, "reason": pb_reason, "coin": coin}
        # PULLBACK_BLOCKER_APPLIED

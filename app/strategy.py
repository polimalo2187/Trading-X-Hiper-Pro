...
ADX_PERIOD = 14
ADX_MIN_TREND_1H = 12
ADX_MIN_TREND_15M = 10  # se mantiene solo como referencia diagnóstica; no bloquea score
ADX_MIN_FALLBACK_1H = 10
TREND_EMA_BUFFER_PCT = 0.0010  # 0.10%
MIN_EMA_SLOPE_PCT_6H = 0.00025 # legacy: ya no se usa para bloquear
...
TIMING_MAX_EMA_DIST = 0.018       # máx distancia % a EMA20 para no perseguir (late entry)
TIMING_MIN_REBOUND = 0.25         # mínimo 'rebote' para considerar que la continuación reanudó (evita pullback profundo)
TIMING_MAX_REBOUND = 0.92         # máximo 'rebote' antes de considerarlo demasiado tarde (evita rebote tardío)
TIMING_MAX_EXT_PCT = TIMING_MAX_EMA_DIST  # alias (misma idea) para extensión sobre EMA20

TIMING_EMA_TOL_PCT = 0.0090       # tolerancia % alrededor de EMA20 para no quedar fuera por centavos

TIMING_LOOKBACK_5M = 36
TIMING_CONFIRM_BARS_5M = 3
MAX_SCORE = 100
MIN_SCORE_TO_SIGNAL = 72
STRENGTH_MAX = 8.0
STRENGTH_MIN = 0.2
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # Trend gate simplificado:
        # 1) Si ADX 1H es suficiente, pasa.
        # 2) Fallback: permitimos tendencia "usable" con ADX algo menor
        #    siempre que el precio esté claramente del lado correcto de la EMA200.
        # Quitamos el filtro de slope porque estaba bloqueando trades buenos.
        ...
        # Score simplificado: prioriza el régimen 1H.
        # El ADX 15M solo queda como diagnóstico suave; ya no manda la entrada.
        score = max(0.0, min(100.0, 62.0 + (adx1v - float(ADX_MIN_TREND_1H)) * 2.2))
        strength = max(STRENGTH_MIN, min(STRENGTH_MAX, (score / 100.0) * STRENGTH_MAX))

        if float(adx15_last) < float(ADX_MIN_TREND_15M):
            weak_adx15 = True
            score = max(0.0, score - 2.0)
            strength = max(STRENGTH_MIN, strength - 0.12)
        ...
        min_score_required = float(MIN_SCORE_TO_SIGNAL) + (2.0 if weak_adx15 else 0.0)
        if vol_regime == "high":
            min_score_required += 1.5
        elif vol_regime == "extreme":
            min_score_required += 3.0
        if float(score) < min_score_required:
            return {
                "signal": False,
                ...
                "diag": {
                    ...
                    "min_score_required": round(float(min_score_required), 4),
                    "adx1": round(float(adx1v), 4),
                    "adx15": round(float(adx15_last), 4),
                    "weak_adx15": bool(weak_adx15),
                },
            }

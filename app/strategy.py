...
# Universo limpio por defecto. Se puede sobrescribir con ALLOWED_TRADE_SYMBOLS="BTC,ETH,SOL"
_DEFAULT_ALLOWED = {
    "BTC", "ETH", "SOL", "ADA", "APT", "NEAR", "TON", "LINK",
    "AVAX", "SUI", "XRP", "ARB",
}
_ALLOWED_ENV = os.getenv("ALLOWED_TRADE_SYMBOLS", "").strip()
ALLOWED_SYMBOLS = {x.strip().upper() for x in _ALLOWED_ENV.split(",") if x.strip()} or _DEFAULT_ALLOWED
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        if ALLOWED_SYMBOLS and coin.upper() not in ALLOWED_SYMBOLS:
            return {"signal": False, "reason": "SYMBOL_NOT_ALLOWED", "coin": coin}
        ...
        out = {
            ...
            "strategy_model": "breakout_retest_5m_v1",
        }
        if LOG_SIGNAL_DIAGNOSTICS:
            _log(

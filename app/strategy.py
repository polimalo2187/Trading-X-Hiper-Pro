...
def _passes_regime_filters_5m(
    ...
    try:
        ...
        # --- Anti-rebote (pullback): usar la media de Bollinger como frontera de continuación ---
        # Si el precio cruza al lado contrario de la media, suele ser rebote hacia la media.
        ...
        if prev_high is not None and prev_low is not None:
            # Para SHORT: requerimos que el close actual esté por debajo del low previo (continuación real).
            if dir_l == "short" and c_last > prev_low:
                return False, "PULLBACK_NO_BREAK", diag
            # Para LONG: requerimos que el close actual esté por encima del high previo (continuación real).

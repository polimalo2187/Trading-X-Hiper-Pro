...
# ------------------------------
TP_WEAK_ACTIVATE_PRICE = 0.011
TP_BASE_ACTIVATE_PRICE = 0.015
TP_STRONG_ACTIVATE_PRICE = 0.019

TRAIL_WEAK_RETRACE_PRICE = 0.0028
TRAIL_BASE_RETRACE_PRICE = 0.0036
TRAIL_STRONG_RETRACE_PRICE = 0.0045

FORCE_MIN_PROFIT_WEAK = 0.006
FORCE_MIN_PROFIT_BASE = 0.0085
FORCE_MIN_PROFIT_STRONG = 0.0115
...
def _dynamic_trade_management_params(strength: float, score: float, atr_pct: Optional[float] = None) -> Dict[str, Any]:
    ...
    if atr_pct_v > 0:
        ...
        atr_tp_component = atr_pct_v * (1.10 if bucket == "strong" else 0.95 if bucket == "base" else 0.82)
        ...
        force_floor_from_sl = sl_proxy * (0.72 if bucket == "strong" else 0.66 if bucket == "base" else 0.60)
        force_floor_from_atr = atr_pct_v * (0.58 if bucket == "strong" else 0.52 if bucket == "base" else 0.46)
    ...
    tp_activate = _clamp(tp_activate, 0.0105, 0.0300)
    ...
    force_min_profit = _clamp(force_min_profit, 0.0055, 0.0200)

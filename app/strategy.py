...
# AUDITORÍA / LOGS
# =========================
# Log SOLO cuando hay señal (para cazar "entry absurdo" sin spamear)
...
def _log(msg: str):
    # No importamos logger para no tocar otras dependencias
    try:
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # ✅ Auditoría de frescura (esto es CLAVE para “entry absurdo”)
        ...
        # ✅ Log bancario SOLO si hay señal
        if LOG_SIGNAL_DIAGNOSTICS:
            _log(
                f"SIGNAL coin={coin} dir={out['direction']} "
                f"close_5={out['close_5']} entry={out['entry_price']} "

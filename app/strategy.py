...
def _passes_5m_timing_filters(
    ...
    try:
        ...
        if dir_l == "long":
            ...
            # Ruptura real (no wick-only, y lookback mayor para evitar rebote/pullback)
            m_break = int(TIMING_BREAK_LOOKBACK_5M)
            recent_high = float(max(h[-m_break-1:-1])) if len(h) >= m_break + 2 else float(max(h[:-1]))

            # Exigir confirmación por CIERRE (evita falsas rupturas por mecha)
            if float(now_close) <= float(recent_high):
                return False, "NO_BREAK_HIGH", {"recent_high": recent_high, "m": m_break, "now_close": now_close, "now_high": now_high}
        ...
        # Exigir confirmación por CIERRE (evita falsas rupturas por mecha)
        if float(now_close) >= float(recent_low):
            return False, "NO_BREAK_LOW", {"recent_low": recent_low, "m": m_break, "now_close": now_close, "now_low": now_low}

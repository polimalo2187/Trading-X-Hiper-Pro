...
TIMING_MAX_EXT_ATR_5M = 0.90      # no entrar si está demasiado extendido vs EMA20
TIMING_MIN_RETRACE_5M = 0.12      # exigir pullback mínimo para entradas de continuación
TIMING_ATR_PERIOD_5M = 14
TIMING_IMPULSE_ATR_MULT = 1.2
TIMING_MAX_RETRACE = 0.50
TIMING_LOOKBACK_5M = 60
TIMING_CONFIRM_BARS_5M = 3
TIMING_SWING_WINDOW_5M = 24  # ventana para medir impulso reciente
TIMING_BREAK_LOOKBACK_5M = 8  # lookback para ruptura real (evita pullback)
...
def _market_expansion_ok(h5: List[float], l5: List[float], mult: float = 1.10) -> bool:
    """Detecta expansión reciente vs bloque anterior (evita rangos/distribución).
    Compara rango de las últimas 3 velas vs las 3 anteriores.
    """
    try:
        if len(h5) < 8 or len(l5) < 8:
            return True
        last_range = float(max(h5[-3:]) - min(l5[-3:]))
        prev_range = float(max(h5[-6:-3]) - min(l5[-6:-3]))
        if prev_range <= 0:
            return True
        return last_range >= prev_range * float(mult)
    except Exception:
        return True
def _passes_5m_timing_filters(
    ...
    """
    Bloqueador de entradas tardías (timing 5m).

    Objetivo:
    - Medir impulso y pullback sobre el *swing reciente* (no sobre un high viejo del lookback)
    - Evitar entradas por mecha (wick-only) en falsas rupturas
    - Exigir ruptura real de estructura (lookback mayor) para filtrar pullbacks/rebotes
    - Mantener filtro EMA20 para operar solo del lado correcto del micro-trend
    """
    diag: Dict[str, Any] = {}
    try:
        n = len(cl5)
        ...
        # Lado correcto de EMA20 (5m)
        # Lado correcto de EMA20 (5m) con tolerancia mínima para evitar bloqueos por micro-ticks
        tol = float(TIMING_EMA_TOL_PCT_5M)
        if dir_l == "long" and now_close < ema20 * (1.0 - tol):
            return False, "BELOW_EMA20", {"close": now_close, "ema20": ema20, "tol": tol}
        if dir_l == "short" and now_close > ema20 * (1.0 + tol):
            return False, "ABOVE_EMA20", {"close": now_close, "ema20": ema20, "tol": tol}

        # ===== Swing reciente (ventana corta) =====
        # Usamos una ventana corta para que el "impulso" sea el actual, no uno viejo del lookback.
        sw = min(int(TIMING_SWING_WINDOW_5M), len(c) - 2)
        if sw < 10:
            return False, "SWING_TOO_SHORT", {"sw": sw}

        hs = h[-(sw + 1):-1]   # excluye la vela actual
        ls = lo[-(sw + 1):-1]
        cs = c[-(sw + 1):-1]

        if dir_l == "long":
            # Impulso: low -> high posterior dentro del swing window
            ...
            if impulse < need:
                return False, "WEAK_IMPULSE", {"impulse": impulse, "need": need}

            # Pullback desde el high del impulso
            post_lows = ls[idx_high:] if idx_high < len(ls) else [ls[-1]]
            C = float(min(post_lows)) if post_lows else float(cs[-1])
            retrace = (B - C) / impulse if impulse > 0 else 1.0
            if retrace > float(TIMING_MAX_RETRACE):
                return False, "DEEP_PULLBACK", {"retrace": retrace}
            # Entrada de CONTINUACIÓN (no perseguimos ruptura):
            # Exigir un pullback mínimo y que el precio retome (re-acceleration) sin estar extendido.
            min_retrace = float(TIMING_MIN_RETRACE_5M)
            if retrace < min_retrace:
                return False, "NO_PULLBACK", {"retrace": retrace, "min": min_retrace}

            ext = float(TIMING_MAX_EXT_ATR_5M) * atr5
            if (now_close - ema20) > ext:
                return False, "TOO_EXTENDED", {"dist": (now_close - ema20), "max": ext}

            # Señal de reanudación: cierre actual > max(cierres previos 2 velas)
            prev2 = c[-3:-1] if len(c) >= 3 else c[:-1]
            ref = float(max(prev2)) if prev2 else float(c[-2])
            if float(now_close) <= ref:
                return False, "NO_RESUME", {"ref": ref, "now_close": now_close}

            # (Opcional) mantener un filtro suave de expansión para evitar operar en rango
            if not _market_expansion_ok(h, lo, mult=1.05):
                return False, "MARKET_NO_EXPANSION", {"m": m_break}

            # Fase de mercado: exigir expansión reciente (evita romper en rango/distribución)
            if not _market_expansion_ok(h, lo, mult=1.10):
                return False, "MARKET_NO_EXPANSION", {"m": m_break}

            diag.update({
                "impulse": round(impulse, 8),
                "retrace": round(retrace, 4),
                "recent_high": round(float(recent_high), 8),
                "now_high": round(float(now_high), 8),
                "now_low": round(float(now_low), 8),
                "sw": sw,
            })
            return True, "OK", diag
        ...
        C = float(max(post_highs)) if post_highs else float(cs[-1])
        retrace = (C - B) / impulse if impulse > 0 else 1.0
        if retrace > float(TIMING_MAX_RETRACE):
            return False, "DEEP_REBOUND", {"retrace": retrace}

        m_break = int(TIMING_BREAK_LOOKBACK_5M)
        recent_low = float(min(lo[-m_break-1:-1])) if len(lo) >= m_break + 2 else float(min(lo[:-1]))

        # Entrada de CONTINUACIÓN (no perseguimos ruptura): exigir rebound mínimo y reanudación bajista.
        min_retrace = float(TIMING_MIN_RETRACE_5M)
        if retrace < min_retrace:
            return False, "NO_REBOUND", {"retrace": retrace, "min": min_retrace}

        ext = float(TIMING_MAX_EXT_ATR_5M) * atr5
        if (ema20 - now_close) > ext:
            return False, "TOO_EXTENDED", {"dist": (ema20 - now_close), "max": ext}

        prev2 = c[-3:-1] if len(c) >= 3 else c[:-1]
        ref = float(min(prev2)) if prev2 else float(c[-2])
        if float(now_close) >= ref:
            return False, "NO_RESUME", {"ref": ref, "now_close": now_close}
        ...
        diag.update({
            "impulse": round(impulse, 8),
            "retrace": round(retrace, 4),
            "recent_low": round(float(recent_low), 8),
            "now_high": round(float(now_high), 8),
            "now_low": round(float(now_low), 8),
            "sw": sw,
        })
...
def _passes_regime_filters_5m(
    ...
    try:
        ...
        # --- Anti-rebote por media de Bollinger (pullback a la media) ---
        # Si el precio está del lado contrario de la media, es más probable que sea rebote/pullback.
        if dir_l == "short" and c_last > mid:
            return False, "BB_MID_REBOUND", diag
        if dir_l == "long" and c_last < mid:
            return False, "BB_MID_REBOUND", diag

        # --- Confirmación de continuación (anti-pullback): exigir ruptura del extremo previo ---
        # Evita entrar en la primera vela de rebote sin romper el mínimo/máximo anterior.
        try:
            prev_high = float(h[-2])
            prev_low = float(lo[-2])
        except Exception:
        ...
        if prev_high is not None and prev_low is not None:
            if dir_l == "short" and c_last > prev_low:
                return False, "PULLBACK_NO_BREAK", diag
            if dir_l == "long" and c_last < prev_high:
                return False, "PULLBACK_NO_BREAK", diag
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # últimos OHLC 5m para filtros de timing (evita entrar tarde)
        try:
            now_high_5 = float(c5[-1].get("h", 0))
            now_low_5 = float(c5[-1].get("l", 0))
        except Exception:
            now_high_5 = 0.0
            now_low_5 = 0.0

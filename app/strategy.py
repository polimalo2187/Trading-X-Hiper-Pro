...
PRICE_WINDOW = 2   # 🔥 FIX CRÍTICO: 2 ticks reales = señal válida
...
def get_entry_signal(symbol: str) -> dict:
    ...
    # ========================================================
    # FUERZA REAL (SCALPING INSTITUCIONAL)
    # ========================================================

    change = (last_price - old_price) / old_price
    strength = abs(change * 100)

    # 🔥 Clamp REAL (no bloquea trading)
    strength = round(min(max(strength, 0.0015), 8.0), 6)

    # 🔥 Threshold REALISTA (FIX)
    effective_threshold = max(
        ENTRY_SIGNAL_THRESHOLD * 0.60,
        0.0015
    )

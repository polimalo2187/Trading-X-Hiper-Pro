...
ADX_MIN_TREND_1H = 20  # umbral de referencia (ya NO bloquea, suma/resta score)
ADX_MIN_TREND_15M = 16  # umbral de referencia (ya NO bloquea, suma/resta score)
...
TIMING_EMA_TOL_PCT_5M = 0.0020  # 0.20% tolerancia vs EMA20 (reduce BELOW/ABOVE_EMA20)
...
TIMING_MAX_EXT_ATR_5M = 1.6  # permite más extensión sin abrir demasiado (reduce TOO_EXTENDED)
...
def _passes_5m_timing_continuation(
    ...
    try:
        ...
        if dir_l == "long":
            ...
            # Nueva lógica: cierre nuevamente a favor (sin exigir ruptura perfecta)
            prev_close = float(c5[-2])
            if close <= prev_close:
                return False, "NO_FOLLOW_THROUGH", {"prev_close": prev_close, "close": close, **diag}
...
def get_entry_signal(symbol: str) -> dict:
    ...
    try:
        ...
        if ema1_last is None or adx1_last is None:
            return {"signal": False, "reason": "NO_TREND_1H_DATA", "coin": coin}

        # Dirección por EMA200 (tendencia estructural). ADX ya NO bloquea: solo pondera el score.
        direction = "long" if float(cl1[-1]) > float(ema1_last) else "short"

        # Score base (se ajusta por calidad de tendencia)
        score = 85.0
        adx1 = float(adx1_last)
        # Penaliza mercado muerto, bonifica tendencia firme
        if adx1 < 12:
            score -= 12
        elif adx1 < 16:
            score -= 6
        ...
        score = max(60.0, min(95.0, float(score)))
        strength = max(STRENGTH_MIN, min(STRENGTH_MAX, (score / 100.0) * STRENGTH_MAX))

        sl_pct = _compute_sl_pct(h15, l15, cl15)

        # Gate final por score (evita entrar en mercado sin tendencia)
        if float(score) < float(MIN_SCORE_TO_SIGNAL):
            if LOG_SIGNAL_DIAGNOSTICS:
                _log(
                    f"BLOCK coin={coin} dir={direction} reason=SCORE_TOO_LOW "
                    f"score={round(float(score),2)} adx1={round(float(adx1_last),2) if adx1_last is not None else None} "
                    f"adx15={round(float(adx15_last),2) if adx15_last is not None else None}"
                )
            return {"signal": False, "reason": "SCORE_TOO_LOW", "coin": coin, "score": round(float(score),2)}

...
PRICE_WINDOW = 4
INTERNAL_SAMPLING_DELAY = 0.25  # ⬅ CLAVE: micro-lecturas reales
...
def get_entry_signal(symbol: str) -> dict:
    ...
    # 🔥 COLECTA ACTIVA DE MICRO-MOVIMIENTO
    for _ in range(PRICE_WINDOW):
        ...
        time.sleep(INTERNAL_SAMPLING_DELAY)
    ...
    # ========================================================
    # FUERZA REAL (SCALPING MICRO)
    # ========================================================

    change = (last_price - old_price) / old_price
    raw_strength = abs(change) * 100

    strength = round(min(max(raw_strength, 0.0006), 6.5), 6)

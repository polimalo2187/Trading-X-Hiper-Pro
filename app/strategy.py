...
ENTRY_SIGNAL_THRESHOLD = 0.12     # % REAL (ajustado para timeframe corto)
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        if price is None or price <= 0:
            return {"signal": False, "reason": "NO_PRICE"}

        update_price(symbol, price)

        prices = price_buffer.get(symbol)
        if not prices:
            return {"signal": False, "reason": "NO_BUFFER"}

        if len(prices) < PRICE_WINDOW:
            return {
                "signal": False,
                "reason": "WINDOW",
                "window": f"{len(prices)}/{PRICE_WINDOW}",
            }
        ...
        if old_price <= 0:
            return {"signal": False, "reason": "BAD_OLD_PRICE"}
        ...
        return {"signal": False, "reason": "EXCEPTION"}

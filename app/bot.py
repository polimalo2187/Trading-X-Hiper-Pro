...
async def activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not has_accepted_terms(user_id):
        await q.edit_message_text(
            "📜 Debes aceptar las políticas antes de operar.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ver Políticas", callback_data="policies")]])
        )
        return
...
"El trading implica riesgo de pérdida total del capital.\n"
"El bot es una herramienta automática y no asesoría financiera.\n"
"El usuario es responsable de su cuenta y claves.\n"
"Al aceptar, declaras comprender los riesgos."
)

async def policies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    kb=[[InlineKeyboardButton("✅ Aceptar", callback_data="policies_accept")],
...
async def policies_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("✅ Has aceptado las políticas.",reply_markup=main_menu(q.from_user.id))

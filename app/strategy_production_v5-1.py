...
IMPULSE_CLOSE_STRENGTH_5M = 0.65
MAX_SCORE = 100
...
# IMPULSE TRIGGER (5M)
# =========================
def _impulse_trigger_5m(o5, h5, l5, c5, direction: str) -> bool:
    """
    Gatillo conservador: solo permite señal si el último candle 5m
    es un impulso real (rango grande vs ATR + cierre fuerte).
    Fail-safe: si algo falla, NO opera.
    """
    try:
        if not o5 or not h5 or not l5 or not c5:
            return False
        ...
        atr5 = float(_atr(h5, l5, c5, ATR_PERIOD) or 0.0)
        if atr5 <= 0:
            return False

        # Regla 1: tamaño del velón
        if candle_range < atr5 * float(IMPULSE_ATR_MULT_5M):
            return False

        # Regla 2: color correcto
        if direction == "LONG" and c <= o:
            return False
        if direction == "SHORT" and c >= o:
            return False

        # Regla 3: cierre fuerte (evitar mechas)
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # =====================
        if not _impulse_trigger_5m(o5, h5, l5, cl5, direction):
            return {
                "signal": False,
                "reason": "NO_IMPULSE_5M",
                "coin": coin,
            }

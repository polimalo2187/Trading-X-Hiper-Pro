...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        if st1 in ("API_FAIL", "BAD_SYMBOL", "BAD_INTERVAL") or st15 in ("API_FAIL", "BAD_SYMBOL", "BAD_INTERVAL") or st1m in ("API_FAIL", "BAD_SYMBOL", "BAD_INTERVAL"):
            return {
                "signal": False,
                "reason": "CANDLES_FETCH_FAIL",
                "detail": {"1h": st1, "15m": st15, "1m": st1m},
                "coin": coin,
            }

        if not c1 or not c15 or not c1m:
        ...
        stale1m, age1m, t1m = _is_stale(c1m, TF_1M)

        if stale1 or stale15 or stale1m:
            return {
                "signal": False,
                "reason": "STALE_CANDLES",
                "coin": coin,
                "age_s": {"1h": round(age1, 1), "15m": round(age15, 1), "1m": round(age1m, 1)},
                "last_t": {"1h": t1, "15m": t15, "1m": t1m},
            }

        _, h1, l1, cl1, _ = _extract(c1)
        _, h15, l15, cl15, _ = _extract(c15)
        o1, h1m, l1m, cl1m, v1m = _extract(c1m)

        if not cl1 or not cl15 or not cl1m:
        ...
        # =====================
        # NUEVO: Impulse trigger 1M (velón disparado)
        # =====================
        if not _impulse_trigger_1m(o1, h1m, l1m, cl1m, direction):
            return {
                "signal": False,

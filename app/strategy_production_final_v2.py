...
# ------------------------------
TP_WEAK_ACTIVATE_PRICE = 0.0088
TP_BASE_ACTIVATE_PRICE = 0.0105
TP_STRONG_ACTIVATE_PRICE = 0.0125

TRAIL_WEAK_RETRACE_PRICE = 0.0028
TRAIL_BASE_RETRACE_PRICE = 0.0036
TRAIL_STRONG_RETRACE_PRICE = 0.0045

FORCE_MIN_PROFIT_WEAK = 0.0058
FORCE_MIN_PROFIT_BASE = 0.0072
FORCE_MIN_PROFIT_STRONG = 0.0088
...
def _dynamic_trade_management_params(strength: float, score: float, atr_pct: Optional[float] = None) -> Dict[str, Any]:
    ...
    if atr_pct_v > 0:
        ...
        atr_tp_component = atr_pct_v * (0.74 if bucket == "strong" else 0.66 if bucket == "base" else 0.58)
        ...
        tp_activate = min(tp_activate, sl_proxy * (1.34 if bucket == "strong" else 1.26 if bucket == "base" else 1.18), atr_pct_v * (0.96 if bucket == "strong" else 0.88 if bucket == "base" else 0.78) if atr_pct_v > 0 else tp_activate)
    ...
    tp_activate = _clamp(tp_activate, 0.0078, 0.0140)
    ...
    force_min_profit = min(force_min_profit, tp_activate * 0.84)
    force_min_profit = _clamp(force_min_profit, 0.0048, 0.0115)

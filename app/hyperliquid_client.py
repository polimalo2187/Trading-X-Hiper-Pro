        ...
        try:
            with httpx.Client(timeout=timeout, headers={"Content-Type": "application/json"}) as client:
                ...
                if not isinstance(data, dict):
                    raise ValueError("Respuesta no es JSON dict")
                return data
...
def _refresh_market_cache():
    ...
    r = make_request("/v2/hyperliquid/getSymbolInfo", {})
    if r and "symbols" in r:
        try:
            _MARKET_CACHE["mids"] = {str(s["symbol"]): float(s.get("lastPrice", 0)) for s in r["symbols"]}
...
def place_market_order(user_id: int, symbol: str, side: str, qty: float):
    ...
    payload = {"action": action, "nonce": nonce, "signature": signature, "wallet": wallet}

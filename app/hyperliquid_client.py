...
# BANK-GRADE: Reuse Client + Backoff 429 + Retry-After
# ============================================================
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 429:
                ...
                sleep_s = min(float(sleep_s), 15.0)  # cap duro
                sleep_s += random.uniform(0.05, 0.25)

                safe_log(f"⏳ 429 Too Many Requests {endpoint} (attempt {attempt}/{retries}) -> sleep {sleep_s:.2f}s")
                if attempt < retries:
                    time.sleep(sleep_s)
                    continue
                return {}
...
META_TTL = 120.0   # ✅ subido para bajar /info meta (antes 60)
MIDS_TTL = 3.0     # ✅ subido un poco (antes 2)
...
def place_market_order(
    ...
    payload = {
        ...
        "nonce": nonce,
        "signature": signature,
        # ✅ NO forzamos expiresAfter para evitar 422 por schema
    }
    if vault_address:

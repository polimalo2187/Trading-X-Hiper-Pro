...
def get_open_position_size(user_id: int, coin: str) -> float:
    """
    Devuelve el SIZE REAL (abs(szi)) de la posición abierta en el exchange para `coin`.
    Fuente: /info clearinghouseState (misma que get_position_entry_price / has_open_position).

    Retorna:
      - 0.0 si no hay posición o no se puede leer.
    """
    wallet = get_user_wallet(user_id)
    if not wallet:
        return 0.0

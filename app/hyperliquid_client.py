# ============================================================
# HYPERLIQUID CLIENT – Compacto y estable (COMPAT + PRICE RULE FIX)
# Compatibilidad:
...
# FIX:
#   - Precio válido según regla oficial HL:
#       * MAX_DECIMALS(perps)=6
#       * max_px_decimals = 6 - szDecimals
#       * max 5 cifras significativas (sig figs)
# ============================================================
...
def _fmt_price_hl(px: float, sz_decimals: int, is_buy: bool) -> str:
    """
    Regla oficial Hyperliquid para perps (docs):
      - MAX_DECIMALS = 6
      - max_px_decimals = 6 - szDecimals
      - max 5 cifras significativas (sig figs)
      - buy: ROUND_UP, sell: ROUND_DOWN
    """
    MAX_DECIMALS = 6
    ...
    try:
        ...
        # Si >= 1: aplicar 5 sig figs (y limitar decimales)
        if d >= 1:
            int_part = int(d)
            digits_before = len(str(abs(int_part))) if int_part != 0 else 1

            # si ya tiene >=5 dígitos antes del punto -> solo entero
            if digits_before >= 5:
        ...
        # Si < 1: limitar solo por max_px_decimals

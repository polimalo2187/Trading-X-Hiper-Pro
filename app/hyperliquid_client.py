...
# PRODUCCIÓN REAL – ESTABLE (LECTURA + ÓRDENES + SIGN)
# - NO crashea
# - place_market_order devuelve dict (ok/filled) siempre que llegue respuesta
# - payload minimal (evita 422 por schema)
# ============================================================
...
# ------------------------------------------------------------
# Cache: META (coin -> asset index, asset -> szDecimals) y ALLMIDS (coin -> mid)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Formateo estricto (reduce 422 por formato)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Detectar fill de forma simple y segura
# ------------------------------------------------------------
...
def _detect_fill(resp: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        filled = _find_first_number_by_keys(
            resp,
            keys_lower={"filledsz", "fillsz", "filledsize", "filled_size", "filled", "filled_qty", "filledqty"}
        )
...
# ------------------------------------------------------------
# Place market order (Limit IOC agresiva, pero estable)
# - Devuelve dict siempre que haya respuesta del exchange
# ------------------------------------------------------------
...
def place_market_order(
    ...
    base_slip = _default_slippage_for_coin(coin, mid) if slippage is None else float(slippage)
    base_slip = _clamp_slippage(base_slip)

    raw_px = mid * (1 + base_slip) if is_buy else mid * (1 - base_slip)
    ...
    try:
        ...
        return {"ok": False, "filled": False, "reason": "SIGN_ERROR", "error": str(e), "coin": coin}

    # ✅ payload MINIMAL para evitar schema issues (422)
    payload = {
        "action": action,
        "nonce": nonce,
        "signature": signature,
    ...
    if not r:
        safe_log("❌ Order rejected/no response:", coin, side, s_str, "px=", p_str)
        return {
            "ok": False,
            "filled": False,
    ...
    if not fill_info["filled"]:
        # ✅ NO_FILL pero respuesta válida => retornamos dict truthy, no None

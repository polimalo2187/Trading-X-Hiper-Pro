...
def get_open_position_size(user_id: int, coin: str) -> float:
    """Devuelve el size REAL (abs(szi)) de la posición abierta para `coin`.
    Usa /info clearinghouseState.
    """
    wallet = get_user_wallet(user_id)
    if not wallet:
        return 0.0
...
def place_sl_trigger(
    ...
    if st != "ok":
        # ✅ Retry corto (1 vez) por sincronización/tick.
        must_log(f"❌ SL trigger rejected coin={coin} resp={resp}")
        try:
            time.sleep(0.35)
            tick = float(tick_size or 0.0)
            if tick > 0:
                # Alejar 1 tick en dirección segura para aumentar probabilidad de aceptación
                if side.lower() == 'sell':
                    trigger_price_retry = float(trigger_price) + tick
                else:
                    trigger_price_retry = max(0.0, float(trigger_price) - tick)
                trigger_px_str_retry = _format_trigger_px(float(trigger_price_retry), tick_size, reduce_side=side)
                action['orders'][0]['t']['trigger']['triggerPx'] = str(trigger_px_str_retry)
                payload['action'] = action
            resp2 = make_request("/exchange", payload)
            st2, _inner2 = _unwrap_exchange(resp2)
            if st2 != 'ok':
                must_log(f"❌ SL retry rejected coin={coin} resp={resp2}")
                return None
            resp = resp2
        except Exception:
            return None

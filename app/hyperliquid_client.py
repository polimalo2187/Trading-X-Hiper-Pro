...
def place_market_order(
    ...
    if slippage is None:
        slippage = _default_slippage_for_coin(coin, mid)
    ...
    if not r:
        ...
        return {"ok": False, "filled": False, "reason": "EXCHANGE_REJECTED", "coin": coin, "side": side, "sz": s_str, "px": p_str}

    fill_info = _detect_fill(r)

    if not fill_info["filled"]:
        # ✅ IMPORTANTE: devolvemos un dict truthy para que el engine NO lo trate como "rechazo",
        # y pueda decidir: "OPEN sin FIL" (no cuenta trade) sin abortar por if not open_resp.

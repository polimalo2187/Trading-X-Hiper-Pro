...
def place_market_order(
    ...
    for attempt in range(1, total_attempts + 1):
        ...
        if notional < (_MIN_TRADE_NOTIONAL + _MIN_TRADE_BUFFER):
            ...
            s_str2 = _format_size_round(target_sz, sz_decimals, ROUND_UP)
            try:
                if float(s_str2) > 0:
                    s_str = s_str2
                    sz_f = float(s_str)
...
# ------------------------------------------------------------
# ✅ TP/SL Trigger Orders (Exchange-side)
# ------------------------------------------------------------

def _detect_order_accept(resp: Any) -> Dict[str, Any]:
    """
    Parser para órdenes NO-IOC (trigger / resting).
    Aceptamos:
      - FILLED (filledSz > 0)
      - RESTING (la orden quedó colocada en el libro / trigger pendiente)
    Rechazamos:
      - ERROR
    """
    out = {"ok": False, "status": "UNKNOWN", "filled": False, "filled_sz": 0.0, "error": ""}
    ...
    if statuses:
        ...
        if first["kind"] == "error":
            out["status"] = "ERROR"
            out["error"] = (first.get("error") or "").strip()
            return out
...
def place_trigger_order(
    ...
    """
    Coloca una orden TRIGGER (TP/SL) en el exchange.
    - side: "buy" o "sell" (acción al disparar)
    - trigger_price: precio disparador
    - tpsl: "sl" o "tp" (Hyperliquid espera "sl"/"tp")
    - is_market: si True, dispara a mercado (recomendado para SL)
    - reduce_only: True para que SOLO cierre/reduzca (no aumenta exposición)
    """
    wallet = get_user_wallet(user_id)
    private_key = get_user_private_key(user_id)
    if not wallet or not private_key:
    ...
    try:
        ...
        qty_f = 0.0
    try:
        trig_px = float(trigger_price)
    except Exception:
        trig_px = 0.0

    if qty_f <= 0:
        return {"ok": False, "filled": False, "reason": "BAD_QTY", "coin": coin}

    if trig_px <= 0:
        return {"ok": False, "filled": False, "reason": "BAD_TRIGGER_PX", "coin": coin}

    # Redondeos (size y trigger px)
    s_str = _format_size(qty_f, sz_decimals)
    p_str = _format_price_side(trig_px, sz_decimals, is_buy=is_buy)
    ...
    if s_f <= 0 or p_f <= 0:
        return {"ok": False, "filled": False, "reason": "BAD_FORMAT", "coin": coin, "sz": s_str, "px": p_str}

    # Notional mínimo aproximado (con trigger px)
    notional = _to_decimal(s_f) * _to_decimal(p_f)
    if float(notional) + 1e-9 < (_MIN_TRADE_NOTIONAL - _MIN_TRADE_BUFFER):
        return {
            "ok": False,
            "filled": False,
    ...
    action = {
        ...
        "orders": [{
            "a": asset,
            "b": is_buy,
            "p": p_str,   # en trigger orders, p=triggerPx
            "s": s_str,
            "r": bool(reduce_only),
            "t": {"trigger": {"isMarket": bool(is_market), "triggerPx": p_str, "tpsl": str(tpsl)}},
        }],
        "grouping": "na",
    }
    ...
    r = make_request("/exchange", payload)
    det = _detect_order_accept(r)

    if not det.get("ok"):
        must_log(
            f"❌ TRIGGER_ORDER_ERROR coin={coin} side={side} trig_px={p_str} sz={s_str} "
            f"tpsl={tpsl} err={det.get('error')}"
        )
        return {
            "ok": False,
            "filled": False,
    ...
    # RESTING o FILLED
    safe_log(
        f"🟢 TRIGGER_ORDER_OK coin={coin} side={side} trig_px={p_str} sz={s_str} "
        f"tpsl={tpsl} status={det.get('status')}"
    )
    return {
        "ok": True,
        "filled": bool(det.get("filled")),
        "reason": det.get("status") or "OK",
        "coin": coin,
        "side": side,
        "trigger_px": p_str,
        "sz": s_str,
...
def place_stop_loss(
    ...
    """
    SL fijo en el exchange (trigger -> market, reduce-only).
    position_side: "long" o "short" (posición actual).
    """
    ps = str(position_side or "").lower()
    if ps not in ("long", "short"):
        return {"ok": False, "filled": False, "reason": "BAD_POSITION_SIDE"}

    close_side = "sell" if ps == "long" else "buy"
    return place_trigger_order(
        user_id=user_id,
        symbol=symbol,
        side=close_side,
        qty=qty,

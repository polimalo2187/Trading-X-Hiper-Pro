...
# FIX CRÍTICO:
#  - Precio IOC usa BBO real (l2Book): BUY desde best ask / SELL desde best bid
#  - BUY price redondea ROUND_UP (más fill en IOC)
#  - SELL price redondea ROUND_DOWN (más agresivo)
...
def safe_log(*args):
    # Respetamos tu configuración: VERBOSE_LOGS puede estar True en prod.
...
def make_request(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            # Manejo suave de 429/5xx con backoff (sin reventar el bot)
            ...
            # Errores 4xx normales: log + retry estándar
...
# ------------------------------------------------------------
# BBO REAL (l2Book): best bid / best ask
# ------------------------------------------------------------

def get_bbo(symbol: str) -> Tuple[float, float]:
    """
    Devuelve (best_bid, best_ask). Si falla, devuelve (0.0, 0.0).
    """
    coin = norm_coin(symbol)
    if not coin:
        return (0.0, 0.0)
    ...
    try:
        if bids:
            ...
            if isinstance(b0, dict):
                best_bid = float(b0.get("px", 0) or 0)
        if asks:
            a0 = asks[0]
            if isinstance(a0, dict):
                best_ask = float(a0.get("px", 0) or 0)
    except Exception:
        return (0.0, 0.0)
...
def _format_price_side(px: float, sz_decimals: int, is_buy: bool) -> str:
    """
    Mantiene tu lógica de "max 5 sig figs" y "max 6 decimales",
    pero redondea:
      - BUY: ROUND_UP (para cruzar y llenar IOC)
      - SELL: ROUND_DOWN (más agresivo)
    """
    MAX_DECIMALS = 6
...
def _clamp_slippage(x: float) -> float:
    try:
        ...
        x = 0.02
    return max(0.0, min(x, 0.12))  # subimos cap a 12% (memes/spreads)
...
def place_market_order(
    ...
    slippage_step: float = 0.03,    # step más fuerte para salir de spread
):
    wallet = get_user_wallet(user_id)
    ...
    for attempt in range(1, total_attempts + 1):
        # 1) BBO real (preferido)
        best_bid, best_ask = get_bbo(coin)

        # 2) Fallback a mid si no hay libro
        mid = float(get_price(coin) or 0.0)
        if mid <= 0:
            return {"ok": False, "filled": False, "reason": "NO_MID", "coin": coin}

        ref_px = 0.0
        if is_buy and best_ask > 0:
            ref_px = best_ask
        elif (not is_buy) and best_bid > 0:
            ref_px = best_bid
        else:
            ref_px = mid
        ...
        # ✅ clave: si hay BBO, partimos del ask/bid real y lo empujamos con slip
        ...
        if attempt < total_attempts:
            safe_log(
                ...
                f"(attempt {attempt}/{total_attempts}, slip={round(slip*100,2)}%, bbo=({best_bid},{best_ask}), mid={mid}) -> retry"
            )
            try:

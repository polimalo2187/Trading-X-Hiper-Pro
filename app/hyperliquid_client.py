...
# BANK-GRADE: Reuse Client + Backoff 429 + Logs de error SIEMPRE
# ============================================================
...
def safe_log(*args):
    # Logs verbosos SOLO si está permitido
    if VERBOSE_LOGS or not PRODUCTION_MODE:
        print(*args)

def err_log(*args):
    # ✅ ERRORES SIEMPRE (producción real necesita ver motivo exacto)
    try:
        print(*args)
    except Exception:
        pass
...
# ------------------------------------------------------------
# HTTP: Client reutilizable + rate-limit hardening
# ------------------------------------------------------------
...
def make_request(
    ...
    """
    POST JSON robusto.
    - Reusa un solo httpx.Client()
    - Maneja 429 con Retry-After + backoff exponencial con jitter
    - ✅ ERRORES se imprimen SIEMPRE (err_log)
    """
    if timeout is None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 429:
                ...
                sleep_s = ra if ra is not None else (backoff * (2 ** (attempt - 1)))
                sleep_s = min(float(sleep_s), 15.0)
                sleep_s += random.uniform(0.05, 0.25)

                # 429 es error operativo: mostrar SIEMPRE
                err_log(f"⏳ 429 Too Many Requests {endpoint} (attempt {attempt}/{retries}) -> sleep {sleep_s:.2f}s")
            ...
            # ✅ Cualquier 4xx/5xx: log SIEMPRE con body
            if r.status_code >= 400:
                ...
                # no re-raise para no perder control
                if attempt < retries:
                    sleep_s = min(backoff * (2 ** (attempt - 1)), 10.0)
                    sleep_s += random.uniform(0.05, 0.25)
                    time.sleep(sleep_s)
                    continue
                return {}
...
def _refresh_mids_cache():
    ...
    if not isinstance(r, dict):
        err_log("❌ allMids inválido (se esperaba dict):", type(r), r)
        return
...
# ------------------------------------------------------------
# Detectar FILL real (modo seguro)
# ------------------------------------------------------------
...
def _is_exchange_ok(resp: Any) -> bool:
    try:
        ...
        if isinstance(resp, dict):
            if resp.get("error") or resp.get("err") or resp.get("message") == "error":
                return False
...
def place_market_order(
    ...
    # ✅ Validación dura (si esto falla, Hyperliquid suele dar 422)
    try:
        if float(s_str) <= 0 or float(p_str) <= 0:
            err_log("❌ px/sz inválidos tras formateo:", coin, "px=", p_str, "sz=", s_str, "szDecimals=", sz_decimals)
            return None
    except Exception:
        err_log("❌ px/sz inválidos tras formateo (parse fail):", coin, "px=", p_str, "sz=", s_str)
        return None
    ...
    # ✅ AQUÍ está tu caso: /exchange devolvió {} por 429/422/timeout/etc.
    if not r:
        err_log(
            "❌ /exchange respondió vacío (rechazo/no confirmación). "
            f"coin={coin} side={side} sz={s_str} px={p_str} slippage={slippage}"
        )
        return None

    # ✅ SOLO consideramos trade real si hubo FILL

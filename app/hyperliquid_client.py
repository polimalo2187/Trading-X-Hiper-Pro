...
# + NO_FILL RETRY RESPONSABLE (1 vez) con slippage escalonado
# + PRICING MEJORADO: IOC usando L2 BOOK (best bid/ask) para reducir NO_FILL
# ============================================================
...
# ------------------------------------------------------------
# L2 Book (best bid/ask) — reduce NO_FILL en IOC
# ------------------------------------------------------------

_BOOK_CACHE: Dict[str, Any] = {"ts": 0.0, "data": {}}   # coin -> (bid, ask)
BOOK_TTL = 0.5  # 500ms

def _parse_level_price(level_item: Any) -> Optional[float]:
    """
    Acepta:
    - ["0.123", "1000"] o [0.123, 1000]
    - {"px": "..."} o {"p": "..."} etc (por si cambia shape)
    """
    try:
        if isinstance(level_item, (list, tuple)) and len(level_item) >= 1:
            return float(level_item[0])
        if isinstance(level_item, dict):
            for k in ("px", "p", "price"):
                if k in level_item:
                    return float(level_item[k])
        return None
    except Exception:
        return None

def get_best_bid_ask(coin: str) -> Tuple[float, float]:
    coin = norm_coin(coin)
    if not coin:
        return 0.0, 0.0
    ...
    now = time.time()
    with _cache_lock:
        if now - float(_BOOK_CACHE.get("ts", 0.0) or 0.0) < BOOK_TTL:
            cached = _BOOK_CACHE.get("data", {}).get(coin)
    ...
    try:
        # shape típico: {"levels":[ bids, asks ]}
        levels = r.get("levels") if isinstance(r, dict) else None
        ...
        if bids:
            px = _parse_level_price(bids[0])
            best_bid = float(px or 0.0)
        if asks:
            px = _parse_level_price(asks[0])
            best_ask = float(px or 0.0)

        with _cache_lock:
            _BOOK_CACHE["ts"] = now
            _BOOK_CACHE.setdefault("data", {})[coin] = (best_bid, best_ask)
...
# ------------------------------------------------------------
# Detectar FILL real (para NO contar trades fantasmas)
# ------------------------------------------------------------

def _find_first_number_by_keys(obj: Any, keys_lower: set[str]) -> Optional[float]:
    try:
            ...
            for k, v in obj.items():
                if str(k).lower() in keys_lower:
                    try:
                        ...
                        pass
            for v in obj.values():
                got = _find_first_number_by_keys(v, keys_lower)
                if got is not None:
                    return got
        elif isinstance(obj, list):
            for it in obj:
                got = _find_first_number_by_keys(it, keys_lower)
                if got is not None:
                    return got
        return None
    except Exception:
...
def _detect_fill(resp: Any) -> Dict[str, Any]:
    ...
    try:
        # 1) Si hay error evidente
        if isinstance(resp, dict) and (resp.get("error") or resp.get("err")):
            out["status"] = "ERROR"
            return out

        # 2) Buscar filled size numérico en cualquier parte
        filled = _find_first_number_by_keys(
            resp,
            keys_lower={
                "filledsz", "fillsz", "filledsize", "filled_size",
                "filled", "filled_qty", "filledqty"
            }
        )

        # 3) Algunos responses traen statuses con "filled" textual
        status_text = None
        if isinstance(resp, dict):
            for k in ("status", "state", "result"):
                if k in resp:
                    status_text = str(resp.get(k))
                    break
...
# ------------------------------------------------------------
# Slippage adaptativo para IOC (reduce NO_FILL)
# ------------------------------------------------------------

def _default_slippage_for_coin(coin: str, ref_price: float) -> float:
    """
    IOC requiere cruzar spread.
    Con L2Book, no necesitamos slippage tan alto como con mid.
    """
    try:
        if ref_price < 1:
            return 0.01  # 1.0% base
        if ref_price < 10:
            return 0.008  # 0.8% base
        return 0.005      # 0.5% base
    except Exception:
        return 0.01
...
def _clamp_slippage(x: float) -> float:
    try:
        ...
        return 0.01
    return max(0.0, min(x, 0.06))  # max 6%

# ------------------------------------------------------------
# Place market order (Limit IOC agresiva)
# + NO_FILL retry responsable (1 vez) con slippage escalonado
# + Pricing usando L2Book (best bid/ask) para ejecutar mejor
# ------------------------------------------------------------
...
def place_market_order(
    ...
    side: str,
    qty: float,
    slippage: Optional[float] = None,       # None => usa slippage adaptativo
    vault_address: Optional[str] = None,    # si usas vault, aquí
    max_no_fill_retries: int = 1,           # ✅ 1 reintento responsable por defecto
    retry_delay_seconds: float = 0.35,      # ✅ 350ms
    slippage_step: float = 0.01,            # ✅ sube 1.0% en el reintento
):
    wallet = get_user_wallet(user_id)
    ...
    # --- Tomar best bid/ask real ---
    best_bid, best_ask = get_best_bid_ask(coin)

    # fallback a mid si book falla
    mid = get_price(coin)
    if mid <= 0 and (best_bid <= 0 or best_ask <= 0):
        safe_log("❌ No hay precio (mid/book) para:", coin)
        return {"ok": False, "filled": False, "reason": "NO_PRICE"}

    # referencia para slippage base
    ref_price = best_ask if is_buy and best_ask > 0 else (best_bid if (not is_buy) and best_bid > 0 else mid)
    if ref_price <= 0:
        ref_price = mid if mid > 0 else (best_ask if best_ask > 0 else best_bid)

    base_slip = _default_slippage_for_coin(coin, float(ref_price)) if slippage is None else float(slippage)
    ...
    for attempt in range(1, total_attempts + 1):
        ...
        # refrescar book por intento (rápido) para seguir al mercado
        bid, ask = get_best_bid_ask(coin)
        mid_now = get_price(coin)

        # --- pricing inteligente ---
        if is_buy:
            # si hay ask real, cruzamos sobre ese ask
            px_ref = ask if ask > 0 else (mid_now if mid_now > 0 else float(ref_price))
            raw_px = px_ref * (1 + slip)
        else:
            # si hay bid real, cruzamos bajo ese bid
            px_ref = bid if bid > 0 else (mid_now if mid_now > 0 else float(ref_price))
            raw_px = px_ref * (1 - slip)

        p_str = _format_price(raw_px, sz_decimals)
        s_str = _format_size(qty, sz_decimals)

        last_px, last_sz = p_str, s_str

        try:
            if float(s_str) <= 0 or float(p_str) <= 0:
                safe_log("❌ px/sz inválidos tras formateo:", coin, "px=", p_str, "sz=", s_str, "szDecimals=", sz_decimals)
                return {"ok": False, "filled": False, "reason": "BAD_FORMATTED_PX_SZ", "px": p_str, "sz": s_str}
        except Exception:
            safe_log("❌ px/sz inválidos tras formateo (parse fail):", coin, "px=", p_str, "sz=", s_str)
            return {"ok": False, "filled": False, "reason": "BAD_FORMATTED_PX_SZ_PARSE", "px": p_str, "sz": s_str}
        ...
        if fill_info["filled"]:
            ...
            return {
                "ok": True,
                "filled": True,
                "filled_sz": float(fill_info.get("filled_sz", 0.0) or 0.0),
                "reason": "FILLED",
                "coin": coin,
                "side": side,
                "sz": s_str,
        ...
        if attempt < total_attempts:
            safe_log(
                f"🟡 NO_FILL {side.upper()} {s_str} {coin} @~{p_str} "
                f"(attempt {attempt}/{total_attempts}, slip={round(slip*100,2)}%) -> retry"
            )
            try:
        ...
        safe_log(
            f"🟡 NO_FILL {side.upper()} {s_str} {coin} @~{p_str} "
            f"(final {attempt}/{total_attempts}, slip={round(slip*100,2)}%) (IOC cancelado, no se cuenta trade)"
        )
        return {
            "ok": True,
            "filled": False,

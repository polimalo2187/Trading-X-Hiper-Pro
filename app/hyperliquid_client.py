# ============================================================
# HYPERLIQUID CLIENT – Compacto y estable (COMPAT READY + TICK FIX)
# Compatibilidad:
...
# FIX:
#   - Precio válido usando pxDecimals (tick size) del universe
# ============================================================
...
# ---------------------------
# Meta cache: asset index + szDecimals + pxDecimals
# ---------------------------
_META = {"coin_to_asset": {}, "asset_to_sz": {}, "asset_to_px": {}, "ts": 0.0}
...
def _refresh_meta() -> None:
    ...
    for i, item in enumerate(universe):
        ...
        # ✅ pxDecimals es el tick size del precio
        try:
            pxd = int(item.get("pxDecimals", 6))
        except Exception:
            pxd = 6

        asset_to_sz[i] = max(szd, 0)
        asset_to_px[i] = max(pxd, 0)

    with _meta_lock:
        _META["coin_to_asset"] = coin_to_asset
        _META["asset_to_sz"] = asset_to_sz
        _META["asset_to_px"] = asset_to_px
        _META["ts"] = now
...
def get_px_decimals(asset_index: int) -> int:
    _refresh_meta()
    with _meta_lock:
        return int(_META["asset_to_px"].get(asset_index, 6) or 6)
...
# ✅ EXPORT público (tu engine lo importa)
...
def _fmt_price(px: float, px_decimals: int, is_buy: bool) -> str:
    """
    ✅ Respeta tick size del mercado: pxDecimals
    - buy: ROUND_UP para cruzar
    - sell: ROUND_DOWN
    """
    try:
        ...
        if d <= 0:
            return "0"
        q = Decimal("1") if px_decimals <= 0 else Decimal("1").scaleb(-px_decimals)
...
def place_market_order(
    ...
    px_decimals = get_px_decimals(asset)
    ...
    p_str = _fmt_price(raw_px, px_decimals, is_buy=is_buy)

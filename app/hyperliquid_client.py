...
def _find_filled_sz(obj: Any) -> Optional[float]:
    """
    Busca recursivamente campos típicos de fill.
    Si no se puede determinar, retorna None (modo seguro).
    """
    try:
        if isinstance(obj, dict):
            ...
            # a veces viene como lista de statuses
            for v in obj.values():
...
def _is_exchange_ok(resp: Any) -> bool:
    """
    Heurística segura:
    - si hay 'error' evidente -> False
    - si el JSON trae algún fill > 0 -> True
    - si no podemos determinar -> False (no contamos trade)
    """
    try:
        if not resp:
            return False
...
def place_market_order(
    ...
    if not _is_exchange_ok(r):
        safe_log(f"🟡 NO_FILL {side.upper()} {s_str} {coin} @~{p_str} (no se cuenta trade)")
        return None

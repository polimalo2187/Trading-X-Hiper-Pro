...
# Hardening: 429 + 5xx + Circuit Breaker + Reuse Client
# ============================================================
...
def err_log(*args):
    # Errores SIEMPRE (producción real necesita causa exacta)
    try:
        print(*args)
    except Exception:
        pass
...
# ------------------------------------------------------------
# HTTP Client reutilizable + límites (evita tormenta)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Circuit Breaker GLOBAL (para 500/429)
# ------------------------------------------------------------
...
def _cb_active() -> bool:
    with _cb_lock:
        return time.time() < _CB_UNTIL_TS

def _cb_wait_if_needed():
    with _cb_lock:
        until = _CB_UNTIL_TS
        reason = _CB_LAST_REASON
    now = time.time()
    if now < until:
        sleep_s = max(0.0, until - now)
        err_log(f"⛔ CircuitBreaker activo ({reason}) -> sleep {sleep_s:.2f}s")
...
def _sleep_with_jitter(base: float, cap: float = 20.0) -> float:
    s = min(float(base), float(cap))
    s += random.uniform(0.08, 0.35)
    time.sleep(max(0.0, s))
    return s

# ------------------------------------------------------------
# make_request (POST JSON) – HARDENED
# ------------------------------------------------------------
...
def make_request(
    ...
    """
    Robust POST JSON:
    - Circuit breaker global para 429/5xx
    - 429: usa Retry-After si existe, si no backoff exponencial
    - 5xx: backoff exponencial + trip circuit breaker
    """
    if timeout is None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code in (500, 502, 503, 504):
                ...
                base_sleep = min(2.0 + (backoff * (2 ** (attempt - 1))), 18.0)
                # trip breaker para frenar tormenta global
                _cb_trip(seconds=max(4.0, base_sleep), reason=f"{r.status_code} upstream")
            ...
            if r.status_code >= 400:
                ...
                # 4xx normalmente NO se arregla reintentando, pero damos 1-2 chances por red
                if attempt < retries and r.status_code >= 500:
                    base_sleep = min(backoff * (2 ** (attempt - 1)), 10.0)
                    _sleep_with_jitter(base_sleep, cap=15.0)
                    continue
                return {}
            ...
            # red inestable: frenar global un poco
        ...
        if attempt < retries:
            base_sleep = min(1.5 + (backoff * (2 ** (attempt - 1))), 12.0)
            _sleep_with_jitter(base_sleep, cap=15.0)

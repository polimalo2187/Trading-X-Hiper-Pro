...
# --- Market Expansion Filter ---
def _market_expansion_ok(h5, l5):
    try:
        if len(h5) < 8:
            return True
        last_range = max(h5[-3:]) - min(l5[-3:])
        prev_range = max(h5[-6:-3]) - min(l5[-6:-3])
        return last_range > prev_range * 1.15
    except:
        return True
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        if ok5 and not _market_expansion_ok(h5, l5):
            return None, 'MARKET_NO_EXPANSION', {}

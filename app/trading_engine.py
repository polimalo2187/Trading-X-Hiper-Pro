...
LEVERAGE = 5.0         # X3
FIXED_NOTIONAL_USDC = float(os.getenv("FIXED_NOTIONAL_USDC", "3.0"))
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        qty_usdc = float(FIXED_NOTIONAL_USDC)
        # ✅ Modo defensa: notional fijo por operación, independiente del balance
        if qty_usdc < float(MIN_NOTIONAL_USDC):
            log(f"Notional fijo inválido ({qty_usdc} USDC) < {MIN_NOTIONAL_USDC} — skip", "WARN")
            return None
        ...
        log(f"Ejecutando orden {symbol} {side} qty_coin={qty_coin} (fixed_notional~{qty_usdc} USDC, lev={LEVERAGE}x)")

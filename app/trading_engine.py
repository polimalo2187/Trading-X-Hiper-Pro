...
# Versión A: Entrada real → Cierre inmediato con TP real
# ============================================================
...
def generate_signal(symbol):
    """
    Genera señales tipo IA (rango más realista)
    """
...
def execute_trade(user_id, symbol, private_key):
    ...
    # Monto por operación = 20% del capital
    ...
    # ORDEN REAL (Hyperliquid PERP)
    order = place_market_order(user_id, symbol, side, amount, private_key)

    if not order:
        return "❌ Error ejecutando orden real."

    # Pausa mínima para permitir confirmación de Hyperliquid
    time.sleep(0.4)

    # CÁLCULO REAL DE SALIDA BASADO EN TP
    if direction == "long":
    ...
    # FEES
    # --------------------------------------------------------
    owner_fee = round(profit * 0.15, 4)  # tu 15%
    ref_fee = 0

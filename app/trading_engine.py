...
#   - ✅ TP ACTIVA TRAILING EN 2.5% (sin tocar risk.py)
#   - ✅ SI YA ACTIVÓ TRAIL Y RETROCEDE A 2.0% => CIERRA (pérdida de fuerza)
# ============================================================
...
# ✅ TU NUEVA LÓGICA (sin tocar risk.py)
TP_ACTIVATE_TRAIL = 0.025   # 2.5% => activa trailing
TP_RETRACE_EXIT = 0.020     # 2.0% => si ya activó trail y retrocede, cierra
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ NO tocamos risk.py, pero aquí imponemos TU TP mínimo real:
        ...
        while True:
            ...
            if not trailing_active:
                if direction == "long" and price >= tp_min_price:
                    ...
                    best_price = price
                    # trailing desde el pico, pero con piso mínimo de salida (2.0%):
                    trail_from_peak = best_price * (1 - trailing_pct)
                    trailing_stop = max(trail_from_peak, tp_retrace_floor_price)
                    log(f"TRAIL activado {symbol} best={best_price:.8f} stop={trailing_stop:.8f} (tp_min=2.5%, piso=2.0%)", "INFO")

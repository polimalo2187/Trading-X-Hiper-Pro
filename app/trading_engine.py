...
IDLE_RETRY_SECONDS = 2   # ⬅ FIX CRÍTICO (antes 15)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    return {
        "event": "BOTH",
        "open": {"message": f"📈 Trade abierto en {symbol}"},
        "close": {"message": f"📉 Trade cerrado ({reason}) | PnL: {profit}"}
      }

...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    tp_min = float(risk.get("tp_min", risk.get("tp", 0.012)) or 0.012)
    sl_pct = float(risk.get("sl", 0.008) or 0.008)
    trailing_pct = float(risk.get("trailing_pct", 0.007) or 0.007)
    ...
    while True:
        ...
        # activar trailing cuando llega a TP mínimo

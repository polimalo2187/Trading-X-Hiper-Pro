...
# + ROTACIÓN ANTI-PEGADO (por señal inválida / sin señal)
# ============================================================
...
# ✅ ANTI-PEGADO (conservador, NO agresivo)
# Si no hay señal o la señal es inválida por mucho tiempo, rota.
NO_SIGNAL_MAX_SECONDS = 20 * 60   # 20 minutos
NO_SIGNAL_MAX_COUNT = 200         # o 200 ciclos (según tu loop)

symbol_blacklist: dict[str, datetime] = {}
symbol_cooldown: dict[str, datetime] = {}
user_last_symbol: dict[int, str] = {}

user_next_trade_time: dict[int, datetime] = {}   # user_id -> datetime
user_trade_counter: dict[int, dict] = {}         # user_id -> {"hour_key": str, "hour_count": int, "day": date, "day_count": int}

# Estado “pegado” por usuario
user_symbol_state: dict[int, dict] = {}          # user_id -> {"symbol": str, "since": datetime, "count": int}
...
# - soporta wrapper nuevo: dict con "filled"
# - fallback legacy: busca filledSz/fillSz/filled etc.
# ============================================================

def _has_positive_fill(obj: Any) -> bool:
    try:
        # ✅ soporte wrapper nuevo del cliente
...
# ============================================================
# ANTI-PEGADO (rotación conservadora)
# ============================================================

def _clear_preferred_if_same(user_id: int, symbol: str):
    try:
        if user_last_symbol.get(user_id) == symbol:
...
def _register_no_signal_and_maybe_rotate(user_id: int, symbol: str, reason: str):
    """
    - Suma ciclos sin señal/ inválida.
    - Si excede umbral conservador -> cooldown + borra preferred + reset estado.
    """
    st = _touch_user_symbol_state(user_id, symbol)
    st["count"] = int(st.get("count", 0) or 0) + 1

    since = st.get("since") or datetime.utcnow()
    elapsed = (datetime.utcnow() - since).total_seconds()

    if st["count"] >= NO_SIGNAL_MAX_COUNT or elapsed >= NO_SIGNAL_MAX_SECONDS:
        log(
            f"Rotación anti-pegado: {symbol} sin señal/ inválida "
            f"(count={st['count']}, elapsed={int(elapsed)}s, reason={reason}) -> cooldown + rotate",
            "WARN"
        )
        _set_cooldown(symbol)
        _clear_preferred_if_same(user_id, symbol)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    if not raw_symbol:
        log("Símbolo inválido desde scanner", "ERROR")
        return None

    symbol = raw_symbol if raw_symbol.endswith("-PERP") else f"{raw_symbol}-PERP"
    symbol_for_exec = _norm_coin(symbol)

    # Guardamos preferred SOLO como “intento actual”
    user_last_symbol[user_id] = symbol

    log(f"Símbolo elegido: {symbol}")

    if is_blacklisted(symbol):
        log(f"Símbolo {symbol} en blacklist", "INFO")
        _clear_preferred_if_same(user_id, symbol)
        return None

    if _is_in_cooldown(symbol):
        log(f"Símbolo {symbol} en cooldown", "INFO")
        _clear_preferred_if_same(user_id, symbol)
        return None

    # Estado anti-pegado (usuario/símbolo)
    ...
    # ✅ FIX CLAVE: si la señal es inválida, ROTAMOS (evita pegado eterno)
    if not isinstance(signal, dict):
        log(f"Señal inválida {symbol} (strategy no devolvió dict)", "ERROR")
        _register_no_signal_and_maybe_rotate(user_id, symbol, reason="INVALID_SIGNAL")
        return None

    if not signal.get("signal"):
        reason = str(signal.get("reason") or "NO_SIGNAL")
        if signal.get("window") is not None:
            log(f"Sin señal {symbol} reason={reason} window={signal.get('window')}", "INFO")
        elif signal.get("strength") is not None:
            log(f"Sin señal {symbol} reason={reason} strength={signal.get('strength')}", "INFO")
        else:
            log(f"Sin señal {symbol} reason={reason}", "INFO")

        _register_no_signal_and_maybe_rotate(user_id, symbol, reason=reason)
        return None

    # Si llegó aquí, hay señal -> reseteamos estado anti-pegado
    _reset_user_symbol_state(user_id)
    ...
    # consumimos símbolo preferido para evitar “pegado”
    _clear_preferred_if_same(user_id, symbol)
    ...
    log(f"SEÑAL CONFIRMADA {symbol} {direction.upper()} strength={signal.get('strength')}")
    ...
    # ✅ PnL en USDC usando NOTIONAL (qty_usdc), no qty_coin
    profit = round(

...
# ✅ SL por fuerza
SL_NORMAL_PRICE = 0.0200   # 2.00% precio  (~6% ROE @3x)
SL_STRONG_PRICE = 0.0330   # 3.30% precio  (~9.9% ROE @3x)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ TP activa trailing (1.0% por precio) + SL por fuerza (por precio)
        tp_activate_price = float(TP_ACTIVATE_TRAIL_PRICE)
        tp_activate_roe = _pricepct_to_roe(tp_activate_price)  # solo para logs

        if strength >= STRENGTH_STRONG_THRESHOLD:
            sl_price = float(SL_STRONG_PRICE)
        else:
            sl_price = float(SL_NORMAL_PRICE)
        sl_roe = _pricepct_to_roe(sl_price)  # solo para logs
        ...
        best_pnl_pct = 0.0
        trailing_stop_pnl = None  # umbral en ROE
        ...
        while True:
            ...
            if not trailing_active:
                if roe >= tp_activate_roe:
                    ...
                    trailing_stop_pnl = best_pnl_pct - _pricepct_to_roe(TRAIL_RETRACE_PRICE)
                    log(
                        f"TRAIL activado {symbol} roe={roe*100:.2f}% "
                        f"best_roe={best_roe*100:.2f}% stop_roe={trailing_stop_pnl*100:.2f}% "
                ...
                if trailing_stop_pnl is not None and roe <= trailing_stop_pnl:
                    exit_price = price
                    exit_pnl_pct = pnl_pct
                    exit_reason = "TRAIL"
                    break

...
# - Warm-up al arrancar (evita trades fantasmas tras deploy)
# - SL + TP MIN + TRAILING DINÁMICO (desde risk.py)
# - Cuenta trades SOLO si hubo FIL real
# - Rotación responsable si se queda pegado sin señal
# - Tamaño de posición usa el capital configurado en Telegram
# ============================================================
...
# ============================================================
# WARM-UP (anti “órdenes inmediatas” tras deploy/restart)
# ============================================================

ENGINE_START_TIME = time.time()
ENGINE_WARMUP_SECONDS = 120  # 2 minutos
...
user_trade_counter: dict[int, dict[str, Any]] = {}

# Estado de “pegado” por usuario
user_symbol_state: dict[int, dict[str, Any]] = {}
...
# - Soporta:
#   a) respuesta RAW antigua (dict/list) con filledSz/filled etc
#   b) wrapper nuevo: {"ok": True, "filled": True/False, ...}
# ============================================================
    ...
    try:
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                ...
                if lk in ("filledsz", "fillsz", "filledsize", "filled_size", "filled", "filled_qty", "filledqty"):
                    try:
                        fv = float(v)
                        if fv > 0:
                            return True
...
# ============================================================
# ROTACIÓN RESPONSABLE (evita “pegado”)
# ============================================================
...
def _register_no_signal_and_maybe_rotate(user_id: int, symbol: str, reason: Optional[str] = None):
    ...
    if st["count"] >= NO_SIGNAL_MAX_COUNT or elapsed >= NO_SIGNAL_MAX_SECONDS:
        log(
            f"Rotación responsable: {symbol} sin señal (count={st['count']}, elapsed={int(elapsed)}s, reason={reason}) -> cooldown + rotate",
            "INFO"
        )
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    # ✅ Warm-up: al arrancar/deploy, no operamos inmediatamente
    if time.time() - ENGINE_START_TIME < ENGINE_WARMUP_SECONDS:
        return None
    ...
    # ✅ Capital configurado en Telegram (NO el total del exchange)
    capital = float(get_user_capital(user_id) or 0.0)
    log(f"Capital configurado (Telegram): {capital}")

    # ✅ Balance REAL en exchange solo como “seguro” (si es 0, no operar)
    ...
    if direction not in ("long", "short"):
        log(f"Dirección inválida: {direction}", "ERROR")
        _set_cooldown(symbol)
        return None
    ...
    log(f"SEÑAL CONFIRMADA {symbol} {direction.upper()} strength={strength}")
    ...
    # ✅ position_size viene en USDC (notional) basado en capital configurado
    qty_usdc = float(risk.get("position_size", 0.0) or 0.0)
    if qty_usdc <= 0:
    ...
    # ✅ Convertir notional USDC a qty_coin usando mid actual
...
while True:
        ...
        # activar trailing al llegar a TP mínimo
    ...
    # PnL aproximado (sobre el notional USDC configurado)
    profit = round(
    ...
    admin_fee = max(profit, 0.0) * float(OWNER_FEE_PERCENT)

    referrer_id = get_user_referrer(user_id)
    if referrer_id and admin_fee > 0:
        ref_fee = admin_fee * float(REFERRAL_FEE_PERCENT)

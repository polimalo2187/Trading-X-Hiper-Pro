...
# 🔥 FIX CLAVE: control de rotación de símbolos
...
def execute_trade(user_id: int):
    ...
    while True:
        ...
        # ====================================================
        # 2) SCAN MERCADO (CON MEMORIA Y ROTACIÓN REAL)
        # ====================================================
        ...
        if not signal or not signal.get("signal"):
            failed_attempts[symbol] = failed_attempts.get(symbol, 0) + 1
            log(f"⚠️ Sin señal válida en {symbol} ({failed_attempts[symbol]})")

            if failed_attempts[symbol] >= MAX_FAILED_ATTEMPTS:
                log(f"🔄 Rotando símbolo tras {MAX_FAILED_ATTEMPTS} intentos fallidos")
                failed_attempts.pop(symbol, None)
                current_symbol = None

            time.sleep(IDLE_RETRY_SECONDS)
            continue

        # Señal válida → reset de fallos
        failed_attempts.pop(symbol, None)

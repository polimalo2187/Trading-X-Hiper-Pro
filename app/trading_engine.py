...
def _active_trade_state_path(user_id: int) -> str:
    ...
    return os.path.join(ACTIVE_TRADE_STATE_DIR, f"{safe_user_id}.json")


def _persist_active_trade_snapshot(user_id: int, trade_data: dict[str, Any]) -> None:
    try:
        os.makedirs(ACTIVE_TRADE_STATE_DIR, exist_ok=True)
        ...
        log(f"persist_active_trade_snapshot error user={user_id} err={e}", "ERROR")
...
def _load_persisted_active_trade_snapshot(user_id: int) -> Optional[dict[str, Any]]:
    try:
        ...
        log(f"load_persisted_active_trade_snapshot error user={user_id} err={e}", "WARN")
        return None
...
def _delete_persisted_active_trade_snapshot(user_id: int) -> None:
    try:
        ...
        log(f"delete_persisted_active_trade_snapshot error user={user_id} err={e}", "WARN")
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    ...
    if not active_trade:
        ...
        _set_active_trade(user_id, active_trade)

...
from app.hyperliquid_client import place_market_order, get_price, get_balance, has_open_position, get_position_entry_price, get_open_position_size
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ SL2 (emergencia) virtual: 4% ROE @ x3 => 1.333333% de precio
        # (No usamos ROE en el bot; esto es % de precio fijo)
        sl2_pnl_pct = float(SL_MAX_PRICE)

        log(f"Riesgo fijo por precio: TP activa trailing={tp_activate_price:.6f}, SL={sl_price_pct:.6f} (SL2={sl2_pnl_pct:.6f})", "INFO")

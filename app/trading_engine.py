...
def _resp_ok(resp: Any) -> bool:
    # ✅ Cliente nuevo retorna dict con "ok"
    if isinstance(resp, dict) and "ok" in resp:
        return bool(resp.get("ok"))
    # fallback: si no trae ok, asumimos que "llegó algo" => ok
    return bool(resp)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    # ✅ Convertir NOTIONAL (USDC) a qty_coin usando precio actual (mid)
    ...
    # ✅ Si el cliente dice ok=False => reject/error exchange (no es NO_FILL)
    ...
    # ✅ ok=True pero no hubo fill (IOC cancelado, spread, etc.)

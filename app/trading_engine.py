...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        best_pnl_pct = 0.0
        trailing_stop_pnl = None  # umbral en pnl_pct
        ...
        while True:
            ...
            if not trailing_active:
                if pnl_pct >= tp_min:
                    ...
                    trailing_stop_pnl = best_pnl_pct - TRAIL_RETRACE_PP
                    log(
                        f"TRAIL activado {symbol} pnl={pnl_pct*100:.2f}% "
                        f"best={best_pnl_pct*100:.2f}% stop={trailing_stop_pnl*100:.2f}% "
                        f"(activa=+{tp_min*100:.2f}%, retrace={TRAIL_RETRACE_PP*100:.2f}pp)",
                        "INFO",
                    )
            else:
                # ✅ actualizar máximo

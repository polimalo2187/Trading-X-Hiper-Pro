...
TP_ACTIVATE_TRAIL_PRICE = 0.010000  # ACTIVA TRAILING 1.00% PRECIO (x3 incluido)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ SL FIJO (FSL) 100% por precio — SIEMPRE 1.17%.
        # No depende del TP ni de ROE. (ROE es solo tu referencia mental.)
        SL_PRICE_PCT = 0.0117
        ...
        # ============================================================
        # ✅ SIZE REAL (EXCHANGE) — CLAVE ANTI-ÓRDENES RIDÍCULAS
        # Hyperliquid puede ajustar el fill/size (step size, fills parciales, etc.).
        # Desde aquí en adelante, SIEMPRE usamos el size real del exchange para:
        #   - SL real (qty)
        #   - CLOSE (qty)
        #   - Registro en DB
        #   - Cálculo de PnL
        ...
        if real_qty <= 0:
            log("CRITICAL: size real inválido tras OPEN — abortando", "CRITICAL")
            # Best-effort: cerrar con el qty solicitado (si llegó a abrir algo)
            try:
        ...
        real_notional = float(real_qty) * float(entry_price)
        MIN_NOTIONAL_USDC = 10.0
        if real_notional < MIN_NOTIONAL_USDC:
            log(f"⚠️ Posición demasiado pequeña (dust) size={real_qty} ntl~{round(real_notional,4)} — cerrando", "WARN")
            try:
        ...
        log(f"Size real (exchange): {real_qty} | notional~{round(real_notional,4)} USDC", "INFO")
...
# ✅ SL REAL EN EXCHANGE (protección aunque el bot se caiga)
        ...
        if exit_reason == "SL_EXCHANGE":
            ...
            close_resp = place_market_order(user_id, symbol_for_exec, opposite, real_qty)
...
# ✅ Profit usando %PnL real calculado en el loop (no dependemos de UI del exchange)
        profit = round(float(exit_pnl_pct) * float(real_notional), 6)

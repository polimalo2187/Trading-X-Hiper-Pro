...
ADOPT_EMERGENCY_SL_PCT = min(float(ADOPT_EMERGENCY_SL_PCT_RAW), float(ENGINE_MAX_SL_PRICE_PCT))  # nunca superar el cap de seguridad del engine
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    ...
    if float(ADOPT_EMERGENCY_SL_PCT) < float(ADOPT_EMERGENCY_SL_PCT_RAW):
        log(
            f"ADOPT SL capado por seguridad del engine: raw={float(ADOPT_EMERGENCY_SL_PCT_RAW):.6f} -> final={float(ADOPT_EMERGENCY_SL_PCT):.6f}",
            "WARN",
        )

...
#   - ✅ TP ACTIVA TRAILING EN 1.0% (sin tocar risk.py)
#   - ✅ SI YA ACTIVÓ TRAIL Y RETROCEDE A 0.8% => CIERRA (pérdida de fuerza)
#   - ✅ SL FIJO EN 2.0% (precio) (sin tocar risk.py)
# ============================================================
...
# ✅ TU LÓGICA ACTUAL (sin tocar risk.py)
TP_ACTIVATE_TRAIL = 0.010   # 1.0% => activa trailing
TP_RETRACE_EXIT   = 0.008   # 0.8% => si ya activó trail y retrocede, cierra
SL_FIXED_PCT      = 0.020   # 2.0% => SL fijo por precio
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # SL + TP mínimo + trailing + TP hard max
        if direction == "long":
            sl_price = entry_price * (1 - sl_pct)
            tp_min_price = entry_price * (1 + tp_min)
            tp_retrace_floor_price = entry_price * (1 + TP_RETRACE_EXIT)
        ...
        while True:
          ...
          # ✅ TP HARD MAX (25%): cierra sí o sí
            ...
            # ✅ activar trailing al llegar TP mínimo (1.0%)
            if not trailing_active:
                if direction == "long" and price >= tp_min_price:
                    trailing_active = True
                    best_price = price
                    # trailing desde el pico, pero con piso mínimo de salida (0.8%)
                    trail_from_peak = best_price * (1 - trailing_pct)
                    trailing_stop = max(trail_from_peak, tp_retrace_floor_price)
                    log(
                        f"TRAIL activado {symbol} best={best_price:.8f} stop={trailing_stop:.8f} "
                        f"(tp_min=1.0%, piso=0.8%)",
                        "INFO"
                    )
                    ...
                    best_price = price
                    trail_from_peak = best_price * (1 + trailing_pct)
                    trailing_stop = min(trail_from_peak, tp_retrace_floor_price)
                    log(
                        f"TRAIL activado {symbol} best={best_price:.8f} stop={trailing_stop:.8f} "
                        f"(tp_min=1.0%, piso=0.8%)",
                        "INFO"
                ...
                # ✅ trailing dinámico + piso (pérdida de fuerza)
                if direction == "long":
                    if price > best_price:
                        best_price = price
                        trail_from_peak = best_price * (1 - trailing_pct)

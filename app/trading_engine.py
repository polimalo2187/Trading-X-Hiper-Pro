...
MAX_TRADE_DURATION_SECONDS = 60 * 10   # 10 minutos máximo por trade
PRICE_CHECK_INTERVAL = 0.5             # segundos
...
def execute_trade(user_id: int):
    ...
    # 6) PRECIO REAL DE ENTRADA (NO SEÑAL)
    entry_price = get_price(symbol)
    if not entry_price:
        return "❌ No se pudo obtener precio real de entrada."
    ...
    if not exit_price:
        return "❌ No se pudo determinar precio de salida."

    # 9) ORDEN DE SALIDA REAL
...
📈 Entrada real: `{entry_price}`
📉 Salida real: `{exit_price}`

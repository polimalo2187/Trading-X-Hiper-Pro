...
# ✅ FSL fijo SIEMPRE por % de precio (sin ROE)
SL_PRICE_PCT = 0.011700
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ Size REAL del exchange (evita "dust" y cierres parciales)
        real_qty = float(get_open_position_size(user_id, symbol_for_exec) or 0.0)
        if real_qty <= 0:
            log("No se pudo leer size real tras OPEN — cerrando por seguridad", "CRITICAL")
            place_market_order(user_id, symbol_for_exec, opposite, qty_coin)
            _register_post_close_cooldown(user_id)
            return None
        qty_coin = real_qty  # a partir de aquí, TODO se hace con size real

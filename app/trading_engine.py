...
TRAIL_RETRACE_PRICE = 0.003000  # CIERRA SI RETROCEDE 0.30% DESDE EL MAX
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ✅ Primero: si ya hay posición abierta, no intentamos nada más.
        #    (En Hyperliquid, el "balance disponible" puede verse como 0 mientras el margen está en la posición)
        ...
        # ✅ Capital efectivo: nunca abrimos con más de lo que el exchange realmente tiene disponible.
        effective_capital = float(min(capital, real_balance))
        log(f"Capital efectivo (min(Telegram, Exchange)): {effective_capital}")

        # ✅ Guard: capital mínimo (evita órdenes ridículas / 0.0)
        if effective_capital < float(MIN_CAPITAL_USDC):
            log(f"Capital insuficiente ({effective_capital} USDC) < {MIN_CAPITAL_USDC} — no se ejecuta trading", "WARN")
            return None
        ...
        risk = validate_trade_conditions(effective_capital, strength)
        if not risk.get("ok"):
        ...
        qty_usdc = float(effective_capital) * float(MARGIN_USE_PCT) * float(LEVERAGE)
        ...
        if not _resp_ok(close_resp):
            ...
            # ✅ Aún si el CLOSE falla, aplicamos cooldown para evitar re-entradas inmediatas (sobre todo tras pérdidas/SL)
            _register_post_close_cooldown(user_id, reason="CLOSE_FAIL")
            return {
                "event": "OPEN",
                "open": {
                    "message": (
        ...
        if not _is_filled_exchange_response(close_resp):
            ...
            # ✅ Cooldown también si no confirmamos fill (evita abrir al instante tras un SL/loss)
            _register_post_close_cooldown(user_id, reason="CLOSE_NOFILL")
            return {
                "event": "OPEN",
                "open": {
                    "message": (

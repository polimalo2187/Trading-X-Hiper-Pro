...
TRAIL_RETRACE_PRICE = 0.005850  # CIERRA SI RETROCEDE 0.585% DESDE EL MAX
...
def _can_trade_now(user_id: int) -> tuple[bool, str]:
    ...
    if STARTUP_GRACE_SECONDS > 0 and user_id not in user_next_trade_time:
        ...
        log(f"Startup grace activo ({STARTUP_GRACE_SECONDS}s, "INFO") para usuario {user_id}")
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # SL por precio: FORZADO FIJO (ignora strategy) para cumplir riesgo del usuario
        sl_price_pct = float(SL_MIN_PRICE)  # SL_MIN_PRICE == SL_MAX_PRICE
        if sl_from_strategy is not None:
            try:
                _sl_dbg = float(sl_from_strategy)
            except Exception:
                _sl_dbg = None
            log(f"SL forzado: {sl_price_pct:.6f} (strategy sugería: {_sl_dbg})", "INFO")

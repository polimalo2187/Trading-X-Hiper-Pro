...
#   - ✅ TP ACTIVA TRAILING EN 1.17% (PRECIO)
#   - ✅ TRAIL REAL: si retrocede 0.585% desde el máximo => CIERRA
#   - ✅ SL FIJO: 1.17% de precio (x3 ya incluido)
#
...
TP_ACTIVATE_TRAIL_PRICE = 0.011700  # 1.17% MOVIMIENTO DE PRECIO (x3 ya incluido)
TRAIL_RETRACE_PRICE    = 0.005850  # 0.585% RETROCESO (mitad de activación)

# ✅ SL por fuerza
SL_MIN_PRICE = 0.011700  # SL FIJO: 1.17% (x3 ya incluido)
SL_MAX_PRICE = 0.011700  # SL FIJO: 1.17% (x3 ya incluido)
...
def _can_trade_now(user_id: int) -> tuple[bool, str]:
    ...
    if STARTUP_GRACE_SECONDS > 0 and user_id not in user_next_trade_time:
        ...
        log.info(f"Startup grace activo ({STARTUP_GRACE_SECONDS}s) para usuario {user_id}")
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        tp_activate_price = float(TP_ACTIVATE_TRAIL_PRICE)  # +1.0% precio activa trailing

        # SL por precio: preferimos el SL recomendado por strategy si viene,
        # pero SIEMPRE lo clamp-eamos al rango oficial [SL_MIN_PRICE, SL_MAX_PRICE].
        sl_from_strategy = None
        try:
            sl_from_strategy = signal.get("sl_price_pct")
            if sl_from_strategy is None:
                sl_from_strategy = signal.get("sl_atr_pct_price")
        except Exception:
            sl_from_strategy = None

        if sl_from_strategy is not None:
            try:
                sl_price_pct = float(sl_from_strategy)
            except Exception:
                sl_price_pct = SL_MIN_PRICE
            sl_price_pct = max(float(SL_MIN_PRICE), min(float(sl_price_pct), float(SL_MAX_PRICE)))
        else:
            # fallback por fuerza
            sl_price_pct = float(SL_MAX_PRICE) if strength >= float(STRENGTH_STRONG_THRESHOLD) else float(SL_MIN_PRICE)

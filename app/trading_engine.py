...
from app.hyperliquid_client import place_market_order, place_sl_trigger, cancel_order_by_oid, get_price, get_balance, has_open_position, get_position_entry_price, get_open_position_size
...
TRAIL_RETRACE_PRICE = 0.002000  # CIERRA SI RETROCEDE 0.20% DESDE EL MAX
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # ============================================================
        # ✅ FIX CRÍTICO (PROD): usar SIZE REAL del exchange
        # El qty calculado (qty_coin) solo sirve para ENVIAR la orden.
        # Para SL/CLOSE/registro, siempre usamos el size real (szi) de la posición.
        # Esto evita "dust positions" (1-2 coins) y cierres parciales indeseados.
        # ============================================================
        real_qty = float(get_open_position_size(user_id, symbol_for_exec) or 0.0)
        if real_qty <= 0:
            log("❌ No se pudo leer size real post-OPEN (szi). Cerrando por seguridad.", "CRITICAL")
            # Fallback: intentar cerrar con qty_coin (best effort)
            place_market_order(user_id, symbol_for_exec, opposite, qty_coin)
            _register_post_close_cooldown(user_id)
            return None

        notional_filled = float(entry_price) * float(real_qty)
        log(f"Size real (exchange): {real_qty} {symbol_for_exec} | notional~{notional_filled:.6f} USDC", "INFO")

        # Anti-dust: si por redondeo/fill queda muy pequeño, cerramos y salimos.
        if notional_filled < float(MIN_NOTIONAL_USDC):
            log(f"🟠 Posición demasiado pequeña (dust) notional~{notional_filled:.6f} < {MIN_NOTIONAL_USDC}. Cerrando.", "WARN")
            place_market_order(user_id, symbol_for_exec, opposite, real_qty)
            _register_post_close_cooldown(user_id)
            return None

# ============================================================
        # ✅ SL REAL EN EXCHANGE (protección aunque el bot se caiga)
        # Usamos el mismo % de PRECIO que ya estabas usando (sl_pnl_pct).
        # LONG: triggerPx = entry*(1 - sl), SHORT: entry*(1 + sl)
        # ============================================================
        try:
            if direction == "long":
                sl_trigger_px = float(entry_price) * (1.0 - float(sl_pnl_pct))
            else:
                sl_trigger_px = float(entry_price) * (1.0 + float(sl_pnl_pct))
        except Exception:
            sl_trigger_px = 0.0
        ...
        if sl_trigger_px <= 0:
            log("No se pudo calcular triggerPx para SL real", "CRITICAL")
            # Por seguridad, cerramos inmediatamente (no dejamos posición sin SL real)
            place_market_order(user_id, symbol_for_exec, opposite, qty_coin)
            _register_post_close_cooldown(user_id)
            return None

        sl_oid = place_sl_trigger(
            user_id=user_id,
            symbol=symbol_for_exec,
            side=opposite,
            qty=real_qty,
            trigger_price=sl_trigger_px,
        )

        if not sl_oid:
            log("❌ No se pudo colocar SL real en el exchange — cerrando posición por seguridad", "CRITICAL")
            place_market_order(user_id, symbol_for_exec, opposite, qty_coin)
            _register_post_close_cooldown(user_id)
            return None
        else:
            log(f"✅ SL real colocado en exchange oid={sl_oid} triggerPx={sl_trigger_px}", "INFO")
        ...
        while True:
            ...
            # ✅ SL REAL en exchange: si la posición ya no existe, el SL (o cierre manual) ocurrió en Hyperliquid.
            if not has_open_position(user_id):
                exit_price = price
        ...
        # ✅ Si SL real cerró la posición en el exchange, NO enviamos CLOSE (ya está cerrada).
        if exit_reason == "SL_EXCHANGE":
            try:
                # Best-effort: si el SL no llegó a llenarse pero la posición se cerró manualmente,
                # cancelamos la orden de SL para no dejar basura.
                cancel_order_by_oid(user_id=user_id, symbol=symbol_for_exec, oid=sl_oid)
            except Exception:
                pass
            close_resp = {"ok": True, "filled": True, "reason": "CLOSED_BY_EXCHANGE_SL"}
        else:
            # ✅ TP/TRAIL virtual: primero cancelamos el SL real en el exchange y luego cerramos la posición.
            try:
                cancel_order_by_oid(user_id=user_id, symbol=symbol_for_exec, oid=sl_oid)
            except Exception:
                pass

            close_qty = float(get_open_position_size(user_id, symbol_for_exec) or 0.0) or float(real_qty)
            close_resp = place_market_order(user_id, symbol_for_exec, opposite, close_qty)
...
# ✅ Profit usando %PnL real calculado en el loop (no dependemos de UI del exchange)
        profit = round(float(exit_pnl_pct) * float(notional_filled), 6)

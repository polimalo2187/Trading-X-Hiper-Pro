...
ADOPT_EMERGENCY_SL_PCT = float(os.getenv("ADOPT_EMERGENCY_SL_PCT", "0.012"))  # fallback solo para ADOPT cuando no existe SL calculado por estrategia
...
def _start_manager_thread(
    *,
    ...
    management_params: Optional[dict[str, Any]] = None,
) -> bool:
    ...
    with _user_manager_guard:
        ...
        _set_active_trade(user_id, {
            ...
            "mode": mode,
            "sl_price_pct": float(sl_price_pct),
            "management_params": _normalize_management_params(management_params, float(entry_strength), float(best_score)),
            "started_at": datetime.utcnow().isoformat(),
        })
...
def _restart_manager_from_active_trade(user_id: int) -> bool:
    ...
    symbol_for_exec = str(active.get("symbol_for_exec") or _norm_coin(symbol.replace("-PERP", "")))
    ...
    mode = str(active.get("mode") or "RECOVER")
    sl_price_pct = float(active.get("sl_price_pct") or ADOPT_EMERGENCY_SL_PCT)
    management_params = active.get("management_params")

    if not symbol or entry_price <= 0 or qty_coin_for_log <= 0:
        log(f"Watchdog manager: estado activo inválido user={user_id} active={active}", "ERROR")
        return False
    ...
    started = _start_manager_thread(
        ...
        mode=f"{mode}_RECOVER",
        sl_price_pct=sl_price_pct,
        management_params=management_params,
    )
    if started:
        log(f"MANAGER watchdog reiniciado user={user_id} symbol={symbol} mode={mode}", "CRITICAL")
    return started
...
def _extract_management_params_from_signal(signal: dict, entry_strength: float, best_score: float) -> dict[str, float]:
    """La estrategia puede entregar TP/trailing/fuerza dinámicos.
    Si no vienen, usamos el fallback histórico del engine para no romper producción.
    """
    fallback = _trade_management_params(float(entry_strength), float(best_score))

    try:
        tp_activate = float(signal.get("tp_activation_price", fallback["tp_activate_price"]) or fallback["tp_activate_price"])
    except Exception:
        tp_activate = float(fallback["tp_activate_price"])

    try:
        trail_retrace = float(signal.get("trail_retrace_price", fallback["trail_retrace_price"]) or fallback["trail_retrace_price"])
    except Exception:
        trail_retrace = float(fallback["trail_retrace_price"])

    try:
        force_min_profit = float(signal.get("force_min_profit_price", fallback["force_min_profit_price"]) or fallback["force_min_profit_price"])
    except Exception:
        force_min_profit = float(fallback["force_min_profit_price"])

    try:
        force_min_strength = float(signal.get("force_min_strength", fallback["force_min_strength"]) or fallback["force_min_strength"])
    except Exception:
        force_min_strength = float(fallback["force_min_strength"])

    bucket = str(signal.get("bucket") or fallback.get("bucket") or "strategy")

    return {
        "bucket": bucket,
        "tp_activate_price": float(tp_activate),
        "trail_retrace_price": float(trail_retrace),
        "force_min_profit_price": float(force_min_profit),
        "force_min_strength": float(force_min_strength),
    }


def _normalize_management_params(mgmt: Optional[dict[str, Any]], entry_strength: float, best_score: float) -> dict[str, float]:
    fallback = _trade_management_params(float(entry_strength), float(best_score))
    if not isinstance(mgmt, dict):
        return fallback

    out = dict(fallback)
    for key in ("bucket", "tp_activate_price", "trail_retrace_price", "force_min_profit_price", "force_min_strength"):
        if key in mgmt and mgmt.get(key) is not None:
            out[key] = mgmt.get(key)

    out["bucket"] = str(out.get("bucket") or fallback["bucket"])
    out["tp_activate_price"] = float(out.get("tp_activate_price") or fallback["tp_activate_price"])
    out["trail_retrace_price"] = float(out.get("trail_retrace_price") or fallback["trail_retrace_price"])
    out["force_min_profit_price"] = float(out.get("force_min_profit_price") or fallback["force_min_profit_price"])
    out["force_min_strength"] = float(out.get("force_min_strength") or fallback["force_min_strength"])
    return out
...
def _manage_trade_until_close(
    *,
    ...
    _ = sl_price_pct
    mgmt = _normalize_management_params(management_params, float(entry_strength), float(best_score))
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    ...
    if not _get_active_trade(user_id):
        _set_active_trade(user_id, {
            ...
            "management_params": _trade_management_params(0.0, 0.0),
            "started_at": datetime.utcnow().isoformat(),
        })
    ...
    adopt_sl_ok = _ensure_exchange_stop_loss(
        ...
        sl_price_pct=float((_get_active_trade(user_id) or {}).get("sl_price_pct") or ADOPT_EMERGENCY_SL_PCT),
        context="ADOPT",
    )
    if adopt_sl_ok:
        log(
            f"🛡️ ADOPT protección SL validada user={user_id} symbol={symbol} dir={direction} entry={float(entry_price):.8f} sl_pct={float((_get_active_trade(user_id) or {}).get('sl_price_pct') or ADOPT_EMERGENCY_SL_PCT):.6f}",
            "WARN",
        )
    else:
        log(
            f"🛡️ ADOPT NO pudo validar/crear SL user={user_id} symbol={symbol} dir={direction} entry={float(entry_price):.8f} sl_pct={float((_get_active_trade(user_id) or {}).get('sl_price_pct') or ADOPT_EMERGENCY_SL_PCT):.6f}",
            "CRITICAL",
        )
    ...
    started = _start_manager_thread(
        ...
        entry_strength=0.0,
        mode="ADOPT",
        sl_price_pct=float(active_trade.get("sl_price_pct") or ADOPT_EMERGENCY_SL_PCT),
        management_params=active_trade.get("management_params"),
    )
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        # Watchdog: si existe una posición/estado activo pero el hilo manager murió, lo levantamos otra vez.
        try:
            if has_open_position(user_id) and not _manager_is_running(user_id):
                _restart_manager_from_active_trade(user_id)
        except Exception as e:
            log(f"Watchdog manager error user={user_id} err={e}\n{traceback.format_exc()}", "ERROR")
        ...
        if not risk.get("ok"):
            ...
            return None
        mgmt = _extract_management_params_from_signal(signal, float(strength), float(signal.get("score", 0.0) or 0.0))
        tp_activate_price = float(mgmt["tp_activate_price"])

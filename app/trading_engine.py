...
# ✅ TP hard máximo (tu “techo”)
TP_HARD_MAX = 0.25     # 25%

# ✅ NUEVA LÓGICA TP/TRAIL (sin tocar risk.py)
TP_ACTIVATE_TRAIL = 0.01      # +1.0% => activa trailing
TRAIL_RETRACE_PP = 0.01       # 1.0pp desde el máximo profit => cierre

# ✅ SL por fuerza (sin tocar risk.py)
SL_NORMAL_PCT = 0.01          # 1.0%
SL_STRONG_PCT = 0.015         # 1.5%
...
def _cooldown_symbol(user_id: int, symbol: str,
                     seconds: int = SYMBOL_NOFILL_COOLDOWN_SECONDS):
    try:
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        if strength < MIN_TRADE_STRENGTH:
            log(
                f"Señal débil bloqueada: strength={strength:.4f} < {MIN_TRADE_STRENGTH}",
                "INFO",
            )
            return None
        ...
        log(
            f"SEÑAL CONFIRMADA {symbol} {direction.upper()} "
            f"strength={signal.get('strength')} score={signal.get('score')}"
        )
        ...
        # ✅ TP activate trail (1%) + SL por fuerza (sin tocar risk.py)
        tp_min = float(TP_ACTIVATE_TRAIL)

        if strength >= STRENGTH_STRONG_THRESHOLD:
            sl_pct = float(SL_STRONG_PCT)
        else:
            sl_pct = float(SL_NORMAL_PCT)

        # (Se deja trailing_pct por compatibilidad, aunque ya no gobierna el cierre.)
        ...
        log(
            f"Ejecutando orden {symbol} {side} qty_coin={qty_coin} "
            f"(notional~{qty_usdc} USDC, lev={LEVERAGE}x)"
        )
        ...
        if not _resp_ok(open_resp):
            ...
            log(
                f"OPEN no OK (reason={reason}) -> cooldown "
                f"{SYMBOL_NOFILL_COOLDOWN_SECONDS}s para {symbol}",
                "ERROR",
            )
            _cooldown_symbol(user_id, symbol, SYMBOL_NOFILL_COOLDOWN_SECONDS)
            return None
        ...
        if not _is_filled_exchange_response(open_resp):
            ...
            log(
                f"OPEN sin FIL (reason={reason}) -> cooldown "
                f"{SYMBOL_NOFILL_COOLDOWN_SECONDS}s para {symbol}",
                "WARN",
            )
            _cooldown_symbol(user_id, symbol, SYMBOL_NOFILL_COOLDOWN_SECONDS)
        ...
        # SL + TP activate trail + TP hard max
        if direction == "long":
            sl_price = entry_price * (1 - sl_pct)
            tp_min_price = entry_price * (1 + tp_min)
            tp_hard_price = entry_price * (1 + TP_HARD_MAX)
        else:
            sl_price = entry_price * (1 + sl_pct)
            tp_min_price = entry_price * (1 - tp_min)
            tp_hard_price = entry_price * (1 - TP_HARD_MAX)
        ...
        while True:
            ...
            # ✅ TP HARD MAX: cierra sí o sí
            if direction == "long" and price >= tp_hard_price:
                exit_price = price
                exit_reason = "TP_HARD"
                break
            if direction == "short" and price <= tp_hard_price:
                exit_price = price
                exit_reason = "TP_HARD"
                break
            ...
            # activar trailing al llegar TP mínimo (+1%)
            if not trailing_active:
                if direction == "long" and price >= tp_min_price:
                    trailing_active = True
                    best_price = price
                    trailing_stop = best_price - (entry_price * TRAIL_RETRACE_PP)
                    log(
                        f"TRAIL activado {symbol} best={best_price:.8f} "
                        f"stop={trailing_stop:.8f} (activa=1.0%, retrace=1.0pp)",
                        "INFO",
                    )
        ...
        if not close_resp:
            log("Orden CLOSE sin respuesta/empty del exchange — NO se registra cierre/PnL",
                "CRITICAL")
            return {
                "event": "OPEN",
                "open": {
                    "message": (
                        ...
                        f"⚠️ Advertencia: cierre NO confirmado por el exchange. "
                        f"Revisa la posición en Hyperliquid."
                    )
                },
            }
        ...
        if not _resp_ok(close_resp):
            ...
            return {
                ...
                "open": {
                    "message": (
                        f"🟡 Trade abierto {symbol} ({direction.upper()})\n"
                        f"⚠️ Advertencia: cierre RECHAZADO por el exchange ({reason}). "
                        f"Revisa la posición en Hyperliquid."
                    )
                },
            }
        ...
        if not _is_filled_exchange_response(close_resp):
            ...
            return {
                ...
                "open": {
                    "message": (
                        f"🟡 Trade abierto {symbol} ({direction.upper()})\n"
                        f"⚠️ Advertencia: cierre sin FIL confirmado ({reason}). "
                        f"Revisa la posición en Hyperliquid."
                    )
                },
            }

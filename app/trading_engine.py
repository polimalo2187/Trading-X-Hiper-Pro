...
from app import hyperliquid_client as hl  # para precio MARK/LAST sin tocar el client
...
# ============================================================
# PRICE SOURCE (para SL/TP)
# ------------------------------------------------------------
# Recomendado: MARK price (evita wicks de LAST y coincide con riesgo/liq).
# Se puede forzar con env RISK_PRICE_SOURCE=mark|last
# ============================================================
RISK_PRICE_SOURCE = os.getenv('RISK_PRICE_SOURCE', 'mark').strip().lower()

def _get_risk_price(symbol: str) -> float:
    """Precio usado para decisiones de SL/TP/TRAIL (MARK recomendado)."""
    src = RISK_PRICE_SOURCE
    try:
        if src == 'last' and hasattr(hl, 'get_last_price'):
            return float(hl.get_last_price(symbol))
        if src == 'mark' and hasattr(hl, 'get_mark_price'):
            return float(hl.get_mark_price(symbol))
        # Fallbacks compatibles con distintas versiones del client
        if hasattr(hl, 'get_price'):
            return float(hl._get_risk_price(symbol))
    except Exception:
        pass
    # Último fallback (lo que ya existía)
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        entry_price_preview = float(_get_risk_price(symbol_for_exec) or 0.0)
        ...
        if entry_state > 0:
            ...
            if entry_fill and entry_fill > 0:
                ...
                entry_price = float(_get_risk_price(symbol_for_exec) or 0.0)
        ...
        while True:
            price = float(_get_risk_price(symbol_for_exec) or 0.0)
            if price <= 0:

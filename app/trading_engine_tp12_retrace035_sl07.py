...
from app.hyperliquid_client import place_market_order, place_stop_loss, cancel_all_orders_for_symbol, get_price, get_balance, has_open_position, get_position_entry_price, get_open_position_size, make_request, get_recent_closed_pnl
...
TP_ACTIVATE_TRAIL_PRICE = 0.012  # ACTIVA TRAILING 1.2% PRECIO (x3 incluido)
TRAIL_RETRACE_PRICE = 0.0035  # CIERRA SI RETROCEDE 0.35% DESDE EL MAX

# ✅ SL por fuerza
SL_MIN_PRICE = 0.007  # SL FIJO 0.7% PRECIO (x3 incluido)
    ...
    *,
    ...
    if size_signed_now == 0.0:
        ...
        try:
            ...
            diag_pnl = get_recent_closed_pnl(user_id, symbol, since_ms=now_ms - 30 * 60 * 1000)
            net = float(diag_pnl.get("net") or 0.0)
            if abs(net) > 0.0:
                profit = round(net, 6)
                log(
                    f"MANAGER[{mode}] {symbol}: PnL_REAL={profit} (fills={diag_pnl.get('fills',0)} fees={diag_pnl.get('fees',0)})",
                    "INFO",
                )
        except Exception as e:
            log(f"MANAGER[{mode}] {symbol}: no pudo leer PnL REAL (fallback) err={e}", "WARN")
    ...
    # --- PnL REAL desde exchange (fills) ---
    profit = None
    try:
        now_ms = int(time.time() * 1000)
        diag_pnl = get_recent_closed_pnl(user_id, symbol, since_ms=now_ms - 20 * 60 * 1000)
        net = float(diag_pnl.get("net") or 0.0)
        if abs(net) > 0.0:
            profit = round(net, 6)
            log(
                f"Trade cerrado (MANAGER[{mode}]) {symbol} PnL_REAL={profit} (fills={diag_pnl.get('fills',0)} fees={diag_pnl.get('fees',0)})",
                "INFO",
            )
    except Exception as e:
        log(f"MANAGER[{mode}] no pudo leer PnL REAL (fallback) err={e}", "WARN")

    if profit is None:
        profit = round(float(exit_pnl_pct) * float(qty_usdc_for_profit), 6)
        log(
            f"Trade cerrado (MANAGER[{mode}]) {symbol} PnL={profit} (pnl_pct={exit_pnl_pct*100:.2f}%)",
            "INFO",
        )

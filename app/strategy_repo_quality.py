...
TIMING_IMPULSE_ATR_MULT = 1.35
TIMING_MAX_RETRACE = 0.45
TIMING_LOOKBACK_5M = 60
TIMING_CONFIRM_BARS_5M = 4
MAX_SCORE = 100
...
def _passes_5m_timing_filters(
    ...
    try:
        ...
        if dir_l == "long":
            ...
            # Permitir entrada más temprana: si el HIGH actual rompe el nivel, aunque el close aún no lo haya superado.
            if (now_close <= float(recent_high)) and (float(now_high) <= float(recent_high)):
                return False, "NO_BREAK_HIGH", {"recent_high": recent_high, "m": m, "now_high": now_high, "now_close": now_close}
            diag.update({"impulse": round(impulse, 8), "retrace": round(retrace, 4), "recent_high": round(float(recent_high), 8), "now_high": round(float(now_high), 8), "now_low": round(float(now_low), 8)})
            return True, "OK", diag
        ...
        # Permitir entrada más temprana: si el LOW actual rompe el nivel, aunque el close aún no lo haya superado.
        if (now_close >= float(recent_low)) and (float(now_low) >= float(recent_low)):
            return False, "NO_BREAK_LOW", {"recent_low": recent_low, "m": m, "now_low": now_low, "now_close": now_close}
        diag.update({"impulse": round(impulse, 8), "retrace": round(retrace, 4), "recent_low": round(float(recent_low), 8), "now_high": round(float(now_high), 8), "now_low": round(float(now_low), 8)})
        return True, "OK", diag
...
def _sma(series, period: int) -> float:
    try:
        ...
        if p <= 0 or len(series) < p:
            return 0.0
        return float(sum(series[-p:])) / float(p)
    except Exception:
        return 0.0
...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        # --- Endurecimiento extra (tendencia pura): vela a favor + fuerza de vela + volumen ---
        try:
            o_last = float(c5[-1].get('o', 0))
            h_last = float(c5[-1].get('h', 0))
            l_last = float(c5[-1].get('l', 0))
            c_last = float(c5[-1].get('c', 0))
        except Exception:
            o_last = h_last = l_last = c_last = 0.0
        rng = max(1e-12, (h_last - l_last))
        body = abs(c_last - o_last)
        body_ratio = body / rng
        # 1) No entrar en vela contraria (evita pullback/rebote inmediato)
        if out_dir == 'long' and c_last <= o_last:
            diag_cc = {'o': o_last, 'c': c_last}
            if LOG_SIGNAL_DIAGNOSTICS: _log(f"BLOCK coin={coin} dir={out_dir} reason=COUNTER_CANDLE diag={diag_cc}")
            return {'signal': False, 'reason': 'TIMING_5M_COUNTER_CANDLE', 'coin': coin, 'diag': diag_cc}
        if out_dir == 'short' and c_last >= o_last:
            diag_cc = {'o': o_last, 'c': c_last}
            if LOG_SIGNAL_DIAGNOSTICS: _log(f"BLOCK coin={coin} dir={out_dir} reason=COUNTER_CANDLE diag={diag_cc}")
            return {'signal': False, 'reason': 'TIMING_5M_COUNTER_CANDLE', 'coin': coin, 'diag': diag_cc}
        # 2) Fuerza mínima de vela (cuerpo/rango)
        if body_ratio < float(MIN_BODY_RATIO):
            diag_wc = {'body': round(body, 10), 'range': round(rng, 10), 'body_ratio': round(body_ratio, 4), 'min_body_ratio': float(MIN_BODY_RATIO)}
            if LOG_SIGNAL_DIAGNOSTICS: _log(f"BLOCK coin={coin} dir={out_dir} reason=WEAK_CANDLE diag={diag_wc}")
            return {'signal': False, 'reason': 'TIMING_5M_WEAK_CANDLE', 'coin': coin, 'diag': diag_wc}
        # 3) Volumen: exigir actividad por encima de la media (evita chop)
        v_ma = float(_sma(v5, int(VOLUME_MA_PERIOD)) or 0.0)
        v_last = float(v5[-1]) if v5 else 0.0
        if v_ma > 0 and v_last < float(VOLUME_MULTIPLIER) * v_ma:
            diag_v = {'v_last': round(v_last, 4), 'v_ma': round(v_ma, 4), 'mult': float(VOLUME_MULTIPLIER)}
            if LOG_SIGNAL_DIAGNOSTICS: _log(f"BLOCK coin={coin} dir={out_dir} reason=LOW_VOLUME_5M diag={diag_v}")
            return {'signal': False, 'reason': 'TIMING_5M_LOW_VOLUME_5M', 'coin': coin, 'diag': diag_v}

...
def _trade_management_params(entry_strength: float, best_score: float) -> dict[str, float]:
    """Fallback legacy si la estrategia no envía parámetros de gestión."""
...
def _coalesce_management_params(
    *,
    ...
    """Prioriza parámetros strategy-driven; si faltan, usa fallback legacy."""
    base = _trade_management_params(float(entry_strength), float(best_score))
    src = signal if isinstance(signal, dict) else (active_trade if isinstance(active_trade, dict) else {})

    def _pick(name: str, fallback: float) -> float:
        try:
            val = float(src.get(name, fallback) or fallback)
            return val if val > 0 else float(fallback)
        except Exception:
            return float(fallback)

    bucket = str(src.get("mgmt_bucket") or src.get("bucket") or base.get("bucket") or "fallback")
    return {
        "bucket": bucket,
        "tp_activate_price": _pick("tp_activation_price", float(base["tp_activate_price"])),
        "trail_retrace_price": _pick("trail_retrace_price", float(base["trail_retrace_price"])),
        "force_min_profit_price": _pick("force_min_profit_price", float(base["force_min_profit_price"])),
        "force_min_strength": _pick("force_min_strength", float(base["force_min_strength"])),
    }
...
def _manage_existing_open_position(user_id: int) -> Optional[dict]:
    ...
    adopt_entry_strength = 0.0
    adopt_best_score = 0.0
    adopt_sl_price_pct = float(ADOPT_EMERGENCY_SL_PCT)
    adopt_mgmt = _coalesce_management_params(entry_strength=0.0, best_score=0.0)
    if isinstance(adopt_signal, dict) and adopt_signal.get("signal") and str(adopt_signal.get("direction") or "").lower() == direction:
        adopt_entry_strength = float(adopt_signal.get("strength", 0.0) or 0.0)
        adopt_best_score = float(adopt_signal.get("score", 0.0) or 0.0)
        adopt_sl_price_pct = float(adopt_signal.get("sl_price_pct", ADOPT_EMERGENCY_SL_PCT) or ADOPT_EMERGENCY_SL_PCT)
        adopt_mgmt = _coalesce_management_params(signal=adopt_signal, entry_strength=adopt_entry_strength, best_score=adopt_best_score)
    ...
    if adopt_sl_ok:
        log(
            f"🛡️ ADOPT protección SL validada user={user_id} symbol={symbol} dir={direction} entry={float(entry_price):.8f} sl_pct={float(adopt_sl_price_pct):.6f}",
            "WARN",
        )
    else:
        log(
            f"🛡️ ADOPT NO pudo validar/crear SL user={user_id} symbol={symbol} dir={direction} entry={float(entry_price):.8f} sl_pct={float(adopt_sl_price_pct):.6f}",
            "CRITICAL",
        )

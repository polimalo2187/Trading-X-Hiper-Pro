...
def _ensure_exchange_stop_loss(
    *,
    ...
    try:
        ...
        if ok:
            log(
                f"{context}: STOP colocado/confirmado en exchange coin={symbol} dir={direction} trigger={sl_resp.get('triggerPx')} qty={sl_resp.get('sz')} status={sl_resp.get('reason')}",
                "WARN",
            )
            return True

        log(
            f"{context}: STOP NO colocado en exchange coin={symbol} dir={direction} trigger~{sl_trigger:.8f} reason={(sl_resp or {}).get('reason') if isinstance(sl_resp, dict) else 'NO_RESP'} err={(sl_resp or {}).get('error','') if isinstance(sl_resp, dict) else ''}",
            "CRITICAL",
        )
        return False
    except Exception as e:

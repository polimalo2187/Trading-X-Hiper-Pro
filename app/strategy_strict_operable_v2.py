...
# =========================
IMPULSE_ATR_MULT_5M = 1.5
IMPULSE_CLOSE_STRENGTH_5M = 0.70

IMPULSE_LOOKBACK_1M = 3  # cuántas velas 1m mirarMAX_SCORE = 100
...
def _impulse_trigger_1m(o, h, l, c, direction: str) -> bool:
    """Gatillo: opera solo si en las ÚLTIMAS N velas de 1m hay un impulso real.
    Esto evita entrar tarde por 1-2 minutos sin bajar demasiado la calidad.
    ...
    try:
        ...
        lookback = int(IMPULSE_LOOKBACK_1M) if "IMPULSE_LOOKBACK_1M" in globals() else 1
        lookback = max(1, min(lookback, len(c)))

        # Recorre las últimas N velas (de más reciente a menos)
        for k in range(1, lookback + 1):
            o1 = float(o[-k]); h1 = float(h[-k]); l1 = float(l[-k]); c1 = float(c[-k])
            rng = h1 - l1
            if rng <= 0:
                continue
            ...
            if direction == "LONG":
                if close_pos >= float(IMPULSE_CLOSE_STRENGTH_5M):
                    return True
            else:
                if close_pos <= (1.0 - float(IMPULSE_CLOSE_STRENGTH_5M)):
                    return True

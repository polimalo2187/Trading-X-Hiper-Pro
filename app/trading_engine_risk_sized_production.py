...
MAX_CAPITAL_RISK_PCT = 0.01  # riesgo máximo por operación: 1% del capital
RISK_BUDGET_BUFFER = 0.90    # reserva para fees/slippage: usa solo 90% del presupuesto de riesgo
# ============================================================
...
def execute_trade_cycle(user_id: int) -> dict | None:
    ...
    try:
        ...
        tp_activate_price = float(mgmt["tp_activate_price"])
        sl_price_pct = float(signal.get("sl_price_pct", 0.0) or 0.0)
        if sl_price_pct <= 0.0:
        ...
        log(
            ...
            f"retrace={float(mgmt['trail_retrace_price']):.6f}, SL(exchange)={sl_price_pct:.6f}, "
            f"force_min_profit={float(mgmt['force_min_profit_price']):.6f}, force_min_strength={float(mgmt['force_min_strength']):.4f}",
            "INFO",
        )

        # ============================================================
        # SIZING POR RIESGO REAL DE CAPITAL (NO MODIFICA EL SL)
        # - La estrategia mantiene su SL dinámico.
        # - El engine ajusta el notional para que la pérdida teórica al SL
        #   no exceda el % máximo del capital operativo.
        # Fórmula aproximada en perp lineal:
        #   loss_usdc ~= notional_usdc * sl_price_pct
        #   notional_max_by_risk = risk_budget_usdc / sl_price_pct
        # ============================================================
        base_qty_usdc = float(capital) * float(MARGIN_USE_PCT) * float(LEVERAGE)
        risk_budget_usdc = float(capital) * float(MAX_CAPITAL_RISK_PCT) * float(RISK_BUDGET_BUFFER)

        if risk_budget_usdc <= 0.0:
            log("risk_budget_usdc inválido", "ERROR")
            return None

        qty_usdc_by_risk = risk_budget_usdc / float(sl_price_pct)
        qty_usdc = min(base_qty_usdc, qty_usdc_by_risk)

        if qty_usdc <= 0.0:
            log(
                f"Sizing inválido por riesgo: capital={capital} risk_budget={risk_budget_usdc} sl_pct={sl_price_pct}",
                "ERROR",
            )
            return None

        est_loss_usdc = qty_usdc * float(sl_price_pct)
        est_loss_pct_capital = (est_loss_usdc / float(capital)) if float(capital) > 0 else 0.0

        log(
            f"Sizing por riesgo: capital={capital:.4f} risk_budget={risk_budget_usdc:.4f} "
            f"base_notional={base_qty_usdc:.4f} risk_notional={qty_usdc_by_risk:.4f} "
            f"final_notional={qty_usdc:.4f} est_loss_usdc={est_loss_usdc:.4f} "
            f"est_loss_pct_capital={est_loss_pct_capital:.6f}",
            "INFO",
        )
        ...
        log(f"Ejecutando orden {symbol} {side} qty_coin={qty_coin} (notional~{qty_usdc:.4f} USDC, base_notional~{base_qty_usdc:.4f} USDC, lev={LEVERAGE}x, risk_cap={MAX_CAPITAL_RISK_PCT*100:.2f}%)")

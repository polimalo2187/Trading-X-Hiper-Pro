...
# ============================================================
# LOG SIMPLE EN VIVO (STDOUT – SERVIDOR)
# ============================================================
...
def create_user(user_id: int, username: str):
    if not users_col.find_one({"user_id": user_id}):
        ...
        db_log(f"👤 Usuario creado {user_id} ({username})")
...
# ============================================================
# VALIDACIÓN CRÍTICA (CON LOGS EN VIVO)
# ============================================================
...
def user_is_ready(user_id: int) -> bool:
    ...
    if not user.get("wallet"):
        db_log(f"[READY] user={user_id} ❌ wallet no configurada")
        return False

    if not user.get("private_key"):
        db_log(f"[READY] user={user_id} ❌ private_key no configurada")
        return False

    if user.get("capital", 0) <= 0:
        db_log(f"[READY] user={user_id} ❌ capital insuficiente ({user.get('capital')})")
        return False
    ...
    db_log(f"[READY] user={user_id} ✅ USUARIO LISTO PARA TRADING")
    return True

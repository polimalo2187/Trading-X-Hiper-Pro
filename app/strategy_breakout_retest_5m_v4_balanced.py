...
def get_entry_signal(symbol: str) -> dict:
    try:
        ...
        vol_quality = 1.0 - _clamp((atr_pct - ATR_PCT_MIN) / max(ATR_PCT_MAX - ATR_PCT_MIN, 1e-12), 0.0, 1.0) * 0.25

        setup_quality = _clamp(
            (0.30 * adx_quality) + (0.24 * slope_quality) + (0.22 * body_quality) + (0.14 * vol_quality) - (0.10 * extension_penalty),
            0.0,
            1.0,
        )
